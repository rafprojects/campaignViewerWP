...
### What to check
- `postmeta` index: `mullion_postmeta_postid_key (post_id, meta_key)`
- `termmeta` index: `mullion_termmeta_termid_key (term_id, meta_key)`
...
```
SHOW INDEX FROM wp_postmeta WHERE Key_name IN ('mullion_postmeta_postid_key');
SHOW INDEX FROM wp_termmeta WHERE Key_name IN ('mullion_termmeta_termid_key');
```
...
### What to check
Endpoint: `GET /wp-json/mullion-gallery/v1/companies/{id}/access?include_campaigns=true`
...
### What to check
Endpoint: `GET /wp-json/mullion-gallery/v1/companies?page=1&per_page=50`

### Expected outcome
- Response includes `X-Mullion-Total`, `X-Mullion-Page`, `X-Mullion-Per-Page` headers.
...
1. Set a low threshold via filter:
	- `add_filter('mullion_slow_query_threshold_ms', fn() => 1);`
2. Call `/campaigns`, `/campaigns/{id}/media`, and `/companies/{id}/access?include_campaigns=true`.
3. Confirm logs in PHP error log contain `"[Mullion] Slow REST"` entries.
