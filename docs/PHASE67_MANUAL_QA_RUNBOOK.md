...
```bash
# Wrap any Mullion call in SAVEQUERIES and print how many DB queries it issued.
npx wp-env run cli wp eval '
  define("SAVEQUERIES", true);
  global $wpdb;
  $before = count($wpdb->queries);
  // …invoke the function under test here, e.g.:
  Mullion_REST_Base::get_accessible_campaign_ids( 1 );
...
| P67-I | Media-library "size" sort uses a real numeric `_mullion_filesize` meta | **Yes** — size sort actually orders by size |
...
- **G-1:** deleted the three deprecated permission primitives `require_campaign_editor()`, `require_campaign_owner()`, `require_space_owner()` from `Mullion_REST_Base` (zero callers since the P53-D role-based rewrite).
- **G-2:** deleted `enrich_media_with_dimensions()` (deprecated wrapper, zero callers), `Mullion_Layout_Templates::check_size_limit()` (private always-`true` no-op, zero callers), and `Mullion_Image_Optimizer::get_stats()` (only its own test called it); relabelled one mislabelled section marker and removed one stray marker + orphaned docblock in the media controller.
- **G-3:** fixed `get_effective_campaign_level()`'s admin short-circuit to resolve the capability against the **passed** `$user_id` (`user_can($user_id, 'manage_mullion')`) instead of the current request user (`current_user_can('manage_mullion')`).
- **E-5:** `mullion_oembed_failure_count`'s two `update_option()` sites now pass `$autoload = false`.
...
grep -rn "enrich_media_with_dimensions\|check_size_limit\|Mullion_Image_Optimizer::get_stats" --include="*.php" .
```

**Expected (pass).** Both greps return nothing. **Why it proves the fix:** any surviving reference would be a call to a now-undefined method — a fatal error waiting to happen. Empty output means the deletions were complete and safe. The `require_space_member` / `require_space_admin` callbacks and `Mullion_Thumbnail_Cache::get_stats()` (a *different*, live, same-named method) are untouched — confirm they still resolve:

```bash
grep -rn "require_space_member\|Mullion_Thumbnail_Cache::get_stats" --include="*.php" includes/ | head
```

**Regression checks.** Full PHP suite green — in particular `Mullion_P52A_Permission_Matrix_Test` (every route still resolves through the map), `Mullion_P33C_Role_Enforcement_Test`, `Mullion_P47_Spaces_Isolation_Test`, and `Mullion_Image_Optimizer_Test` (the two `get_stats` test methods were removed alongside the method). `composer lint:php` stays at 0 findings.
...
**What & why.** The function receives a `$user_id` and gates space access with `can_access_space($space_id, $user_id)`, but its admin short-circuit previously asked `current_user_can('manage_mullion')` — the *current request* user, not the passed one. All (former) callers passed the current user, so there was **no live bug**; the fix removes a latent trap for any future reuse with a different user id.

**Verification: reflection-based unit test, not a REST walkthrough — and here is why.** After G-1 there is **no live REST path** that reaches this private method, so there is no curl sequence that can exercise it. Its behavior is pinned by `Mullion_P64B_Revoke_Granularity_Test` (which reflects into `Mullion_Access_Controller::get_effective_campaign_level`). The meaningful check is that this test passes with the `user_can($user_id, …)` form. To *prove* the fix matters, a reviewer can add a throwaway assertion that passes a **non-current** user id who has `manage_mullion` while the current user does not, and confirm the function returns `'owner'` for the passed user — pre-fix it keyed off the wrong user and returned based on the request user's caps.

**Expected (pass).** `Mullion_P64B_Revoke_Granularity_Test` green. **Why a hand test isn't provided:** there is genuinely no user-facing surface to drive; asserting otherwise would be theater.

#### E-5 — `mullion_oembed_failure_count` no longer autoloads

**Pre-fix behavior.** Both `update_option('mullion_oembed_failure_count', …)` calls omitted the third argument, so the option defaulted to **autoload = yes** — loaded into memory on every single request despite only ever being read inside the oEmbed failure path.

```bash
# Trigger an oEmbed failure so the option gets written (any un-fetchable URL works):
curl -s $AUTH "$BASE/wp-json/mullion-gallery/v1/oembed?url=https://example.com/not-an-oembed" >/dev/null

# Inspect the autoload flag directly:
npx wp-env run cli wp db query \
  "SELECT option_name, autoload FROM wp_options WHERE option_name='mullion_oembed_failure_count'"
# → autoload column reads 'no' (or 'off' on newer WP)
```

**Expected (pass).** `autoload = no`. **Why it proves the fix:** pre-fix the same query returns `autoload = yes`, meaning the counter rode along in `wp_load_alloptions()` on every request. **Pitfall:** on a dev site that recorded a failure *before* this fix, the row may already exist with `autoload = yes`; `update_option(..., false)` flips it on the **next** write, so trigger at least one failure *after* checking out the fix (or `wp option delete mullion_oembed_failure_count` first, then trigger). This is the same self-healing-on-next-write pattern used elsewhere in the plugin.
...
> **Correction (P67-R).** P67-F originally also "fixed" `list_companies()`'s priming call from `mullion_campaign` to `mullion_company`, on the review's premise that it primed the wrong taxonomy. That premise was wrong, and the change was reverted. `update_object_term_cache($ids, $type)`'s second parameter is an **object type** (a post type), which it expands through `get_object_taxonomies()` into *every* taxonomy registered for that type — `mullion_company` included. Passing a taxonomy name makes the whole call a silent no-op, because `get_object_taxonomies('mullion_company') === []`. The original `mullion_campaign` argument was correct all along. See §4.
...
npx wp-env run cli wp eval '
  ...
  Mullion_Campaign_Controller::list_campaigns($req);   // populates transient
  delete_transient_like("mullion_campaigns_");         // force a fresh, uncached build
  $before = count($wpdb->queries);
  Mullion_Campaign_Controller::list_campaigns($req);
...
```bash
curl -s $AUTH "$BASE/wp-json/mullion-gallery/v1/companies" | jq '[.[] | {name, campaignCount: (.campaigns | length)}]'
...
npx wp-env run cli wp eval '
  define("SAVEQUERIES", true);
  global $wpdb;
  $rm = new ReflectionMethod("Mullion_Monitoring", "count_expired_grants_pending_cleanup");
...
npx wp-env run cli wp eval '
  ...
  global $wpdb;
  $before = count($wpdb->queries);
  Mullion_REST_Base::get_accessible_campaign_ids( 1 );   // user 1
  echo "queries: " . (count($wpdb->queries) - $before) . "\n";
'
...
EP=$(curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/webhooks" \
...
CID=$(npx wp-env run cli wp post create --post_type=mullion_campaign --post_title='H timing' --post_status=publish --porcelain)
time curl -s $AUTH -X PUT "$BASE/wp-json/mullion-gallery/v1/campaigns/$CID" \
...
npx wp-env run cli wp cron event run mullion_webhook_retry
curl -s $AUTH "$BASE/wp-json/mullion-gallery/v1/webhooks/deliveries" | jq '.[0] | {event, attempt, success, statusCode}'
...
**Regression checks.** `Mullion_P39IN1_Webhook_Test` passes unmodified — its fixtures save endpoints **without** an `id`, so they exercise the synchronous fallback (`deliver()` is identical in both paths). One **new** test, `test_dispatch_defers_first_attempt_to_cron_for_endpoint_with_id`, covers the fix directly: it asserts the log is empty right after `dispatch()` (no inline delivery), that a `mullion_webhook_retry` event with `attempt = 1` is scheduled, and that running it logs the delivery. This is the first automated coverage of the cron/retry path, which previously had none.

**Pitfall.** Only endpoints created through the REST API (`POST /webhooks`) get a UUID `id` and therefore the cron path; an endpoint written directly into the `mullion_webhook_endpoints` option without an `id` will deliver synchronously. Use the REST endpoint (as above) to exercise the async path.
...
npx wp-env run cli wp eval '
  ...
  update_option(Mullion_Webhooks::LOG_OPTION, []);
  Mullion_Webhooks::dispatch("campaign.created", ["id" => 1, "title" => "veto"]);
...
Pinned by `Mullion_P67R_Review_Fixes_Test::test_dispatch_delivers_inline_when_cron_refuses_the_schedule`.

---

### P67-C — Shared upload error / duplicate response shaping

**What & why.** Two duplications removed: (1) the single-file and batch upload paths each hand-built the same 409 duplicate / near-duplicate field set — now both call one `format_duplicate_fields(WP_Error)` helper; (2) `upload_font()` carried an inline copy of the `$_FILES['error']` → message/status switch that already lived in `get_upload_error_data()` — that helper moved up to `Mullion_REST_Base` and `upload_font()` now calls it.
...
**This is a no-behavior-change refactor — the correct observation is "identical output."** The automated `Mullion_P28N_Duplicate_Detection_Test` pins the single-file and batch 409 field names and is the primary proof. The manual checks below matter most for the **batch near-duplicate** fields, which that test does not pin.
...
CID=$(npx wp-env run cli wp post create --post_type=mullion_campaign --post_title='C dup' --post_status=publish --porcelain)

# Prime: force-upload an image.
curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/media/upload" \
...
curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/fonts/upload" \
  -F "file=@/dev/null;filename=" | jq '{code, message}'
# → mullion_font_upload_failed with the same message text get_upload_error_data() produces
```

**Expected (pass).** `upload_font()` returns `mullion_font_upload_failed` with the message/status the shared `get_upload_error_data()` maps — identical to pre-fix, because the inline switch it replaced was a verbatim copy. **Why it proves the fix:** the error code stays `mullion_font_upload_failed` (the wrapper is caller-specific) while the message/status now come from the single shared source.

**Regression checks.** `Mullion_P28N_Duplicate_Detection_Test` (single + batch 409 field names) passes unmodified; font-upload coverage passes unmodified. **Pitfall:** the batch near-duplicate (`similar_*`) fields are **not** test-pinned — a regression there would pass CI, so the manual near-duplicate check above is the real guard.

---

### P67-D — Unified global-settings write path

**What & why.** `update_settings()`, `patch_settings()`, and the space panel's global-key routing in `update_space_settings()` now all call one `Mullion_REST_Base::write_global_settings($input, $mode, $bump_cache, $summary_prefix, $audit_extra)`. Each keeps its own permission guard and passes a mode (`replace` / `patch` / `changed`) that reproduces its original merge semantics exactly.
...
curl -s $AUTH -X PUT "$BASE/wp-json/mullion-gallery/v1/settings" \
...
curl -s $AUTH -X PATCH "$BASE/wp-json/mullion-gallery/v1/settings" \
...
SID=$(npx wp-env run cli wp eval 'echo Mullion_DB::insert_space(["slug"=>"d-space","name"=>"D","isolation_mode"=>"delegated"]);')
curl -s $AUTH -X PUT "$BASE/wp-json/mullion-gallery/v1/spaces/$SID/settings" \
  -H 'Content-Type: application/json' -d '{"itemsPerPage":30}' >/dev/null
npx wp-env run cli wp eval '$s=get_option("mullion_settings",[]); echo $s["items_per_page"] ?? "unset";'   # → 30
```
...
```bash
# As a NON-manage_options user (a mullion_editor), write a system-level (admin-only)
# global key through BOTH paths and compare the outcomes:
export EDITOR_AUTH='-u editor:APP_PASSWORD'   # a manage_mullion (not manage_options) user

# /settings → explicit 403 naming the blocked field:
curl -s -o /dev/null -w '%{http_code}\n' $EDITOR_AUTH -X PATCH "$BASE/wp-json/mullion-gallery/v1/settings" \
...
curl -s -o /dev/null -w '%{http_code}\n' $EDITOR_AUTH -X PUT "$BASE/wp-json/mullion-gallery/v1/spaces/$SID/settings" \
  -H 'Content-Type: application/json' -d '{"cacheTtl":120}'      # → 200
npx wp-env run cli wp eval '$s=get_option("mullion_settings",[]); echo $s["cache_ttl"] ?? "unset";'   # → unchanged (NOT 120)
```
...
**What & why.** Three small dedups: **(a)** the three `updated`/`added`/`deleted`_post_meta closures that synced `media_items` refs are now one named handler, `mullion_sync_media_refs_on_meta_change()`; **(b)** the Rumble video-ID token is a single constant `Mullion_Provider_Registry::RUMBLE_VIDEO_ID_TOKEN` used by both the provider and the media controller; **(c)** `resolve_user()` vs `search_users()` — **no code change**: their `search_columns` were confirmed already aligned during planning.
...
```bash
CID=$(npx wp-env run cli wp post create --post_type=mullion_campaign --post_title='E sync' --post_status=publish --porcelain)
...
npx wp-env run cli wp db query "SELECT COUNT(*) FROM $(npx wp-env run cli wp eval 'echo Mullion_DB::get_media_refs_table();') WHERE campaign_id=$CID"   # → 1
...
npx wp-env run cli wp db query "SELECT COUNT(*) FROM $(npx wp-env run cli wp eval 'echo Mullion_DB::get_media_refs_table();') WHERE campaign_id=$CID"   # → 2

# DELETE media_items meta → all refs cleared (the preserved [] quirk):
npx wp-env run cli wp post meta delete $CID media_items
npx wp-env run cli wp db query "SELECT COUNT(*) FROM $(npx wp-env run cli wp eval 'echo Mullion_DB::get_media_refs_table();') WHERE campaign_id=$CID"   # → 0
```
...
curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/media/external" \
...
**Expected (pass).** Provider `rumble`, embed URL built from `v72ksce` — identical to pre-fix. **Why it proves the fix:** both the provider's path-extraction regex and the media controller's validation regex now interpolate the same `RUMBLE_VIDEO_ID_TOKEN`; the extracted/validated id must not change. **Load-order note:** the constant lives on the always-loaded `Mullion_Provider_Registry` (not the lazily-loaded `Mullion_Provider_Rumble`) specifically so the media controller can reference it without a fatal — if you see a "class not found" here, that assumption has been broken.
...
for q in \
  ...
  echo "== /campaigns$q =="
  curl -s $AUTH "$BASE/wp-json/mullion-gallery/v1/campaigns$q" | jq '{total, ids: [.items[].id], page, perPage}'
done
```
...
**Regression checks.** `Mullion_P67B_Query_Builder_Test` (new) plus all existing campaign-listing coverage (`Mullion_Campaign_Rest_Test`, `Mullion_P28E_Campaign_Filters_Test`, `Mullion_P52A5b_Campaign_Space_Scoping_Test`, …) pass unmodified.

---

### P67-A — `sanitize_settings()` collapsed into the registry-driven loop

**What & why.** ~52 of the 58 hand-written `if (isset($input['x']))` field blocks were deleted; the generic trailing loop already sanitizes them from `Mullion_Settings_Registry::$valid_options` (enums) and `::$field_ranges` (int/float clamps) plus the default's type (bool casts). Only six genuinely special fields remain hand-written: `api_base` (URL), `card_border_color` (hex with a brand fallback), `typography_overrides`, `viewer_bg_gradient`, `gallery_config`, `card_config` (structured/nested payloads). One range (`rate_limit_requests_per_minute => [0,6000]`) moved into `$field_ranges`. `sanitize_settings()` shrank from ~280 to ~108 lines (~61%).
...
**This is a byte-for-byte no-behavior-change refactor. The automated suite is the real proof.** `Mullion_Settings_Test`, `Mullion_Settings_Extended_Test`, and `Mullion_Settings_Rest_Test` pin the exact sanitized output per field across the full matrix — they are the equivalence oracle and must pass unmodified. A manual end-to-end pass catches anything field-level tests wouldn't:
...
**What & why.** The media-library `size_asc`/`size_desc` sort mapped to `orderby => meta_value_num` on `_wp_attachment_metadata` — a serialized PHP array whose numeric cast is `0` for every row, so the sort was a no-op with arbitrary order. It now sorts on a dedicated numeric `_mullion_filesize` meta, written at upload time (the plugin's own `create_attachment_from_upload()` **and** a new `add_attachment` hook for native/other-plugin uploads) and backfilled for existing attachments by a one-time DB-v17 migration.
...
for sz in 1k 500k 5M; do
  head -c $sz /dev/urandom > /tmp/mullion-$sz.jpg   # (use real images if the pipeline validates)
  curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/media/upload" -F "file=@/tmp/mullion-$sz.jpg" >/dev/null
done

# Ascending by size → smallest first; descending → largest first:
curl -s $AUTH "$BASE/wp-json/mullion-gallery/v1/media-library?sort=size_asc&per_page=100"  | jq '[.items[].filename]'
curl -s $AUTH "$BASE/wp-json/mullion-gallery/v1/media-library?sort=size_desc&per_page=100" | jq '[.items[].filename]'
...
npx wp-env run cli wp post meta get $LAST _mullion_filesize   # → the byte size (was absent pre-fix)
```
...
NID=$(npx wp-env run cli wp media import /tmp/mullion-500k.jpg --porcelain)
npx wp-env run cli wp post meta get $NID _mullion_filesize   # → byte size (hook path)

# (b) Backfill for pre-existing attachments — strip the meta, clear the guard, re-run:
npx wp-env run cli wp post meta delete $NID _mullion_filesize
npx wp-env run cli wp option delete mullion_filesize_backfilled
npx wp-env run cli wp eval 'delete_option("mullion_db_version"); Mullion_DB::maybe_upgrade();'
npx wp-env run cli wp post meta get $NID _mullion_filesize   # → byte size restored (backfill path)
```

**Expected (pass).** All three write paths (plugin upload, `add_attachment` hook, one-time backfill) leave a correct `_mullion_filesize`. Each is exercised independently because none subsumes the others.

**Pitfalls.**
- Attachments that *don't* yet carry `_mullion_filesize` are **not** dropped from the size-sorted list — the query uses an `OR … NOT EXISTS` meta clause so they sort as NULL (first on ASC / last on DESC) rather than vanishing. Do not "simplify" it to a bare `meta_key`, which would silently exclude them. (P67-R specifically tested whether that `OR` clause fans the join out and duplicates un-stamped rows — it does not; leave the clause alone. `DISTINCT` is *not* the fix if you ever think it is: it errors under `ONLY_FULL_GROUP_BY` because `ORDER BY` references a non-selected column.)
...
- Re-observing the backfill needs **both** the guard option cleared (`mullion_filesize_backfilled`) and the DB version reset, exactly like the P66 migration re-runs.

**The backfill is bounded per run and resumes on cron (P67-R).** It originally looped over the entire attachment library in one pass, from `maybe_upgrade()` on `init` — a `filesize()` stat plus a meta write per row. On a large library that can outlive `max_execution_time`, and since `mullion_db_version` is only bumped after `maybe_upgrade()` returns, a timeout there re-enters the whole upgrade path on every subsequent request. It now stamps at most `FILESIZE_BACKFILL_MAX_BATCHES × FILESIZE_BACKFILL_BATCH` (5 × 200 = 1000) attachments per run and schedules `mullion_filesize_backfill` to continue.
...
npx wp-env run cli wp eval '
  ...
  echo "complete flag: " . var_export(get_option("mullion_filesize_backfilled"), true) . "\n";
  echo "continuation queued: " . var_export((bool) wp_next_scheduled(Mullion_DB::FILESIZE_BACKFILL_HOOK), true) . "\n";
'
...
npx wp-env run cli wp cron event run mullion_filesize_backfill
npx wp-env run cli wp option get mullion_filesize_backfilled   # → 1 once every row is stamped
```

**Expected (pass).** With more unstamped attachments than one run's budget, the guard option stays unset and a `mullion_filesize_backfill` event is queued; running it repeatedly finishes the library and sets the guard, after which the hook is not rescheduled. On a small library the whole thing still completes inside `maybe_upgrade()` with no cron event queued at all.

**Regression checks.** `Mullion_P67I_Filesize_Sort_Test` (hook stamp, idempotency, backfill, and the size sort both directions) and `Mullion_P67R_Review_Fixes_Test` (zero-stamp on unreadable files, bounded run + cron resume, hook registration). `Mullion_Cron_Hooks_Test` pins `mullion_filesize_backfill` in the canonical hook list so deactivate/uninstall clear it. Existing media-library listing coverage passes unmodified.
...
```bash
npx wp-env run cli wp eval '
  var_dump(get_object_taxonomies("mullion_company"));   // → array(0) {}  — a taxonomy is NOT an object type
  var_dump(get_object_taxonomies("mullion_campaign"));  // → includes "mullion_company"
'
```
...
npx wp-env run cli wp eval '
  Mullion_REST_Base::clear_accessible_campaigns_cache();
  wp_cache_flush();
  $m = new ReflectionMethod("Mullion_REST_Base", "get_accessible_campaign_ids");
  $m->setAccessible(true);
  $m->invoke(null, 0);
  $id = (int) get_posts(["post_type" => "mullion_campaign", "posts_per_page" => 1, "fields" => "ids"])[0];
  echo "post cache primed: " . var_export(false !== wp_cache_get($id, "posts"), true) . "\n";
  echo "term cache primed: " . var_export(false !== get_object_term_cache($id, "mullion_company"), true) . "\n";
'
...
npx wp-env run cli wp eval '
  ...
  var_dump(get_post_meta($id, "_mullion_filesize", true));   // → string(1) "0"  (pre-fix: "" — never written)
'
```
...
**Regression checks for the whole block.** `Mullion_P67R_Review_Fixes_Test` (7 tests) pins all four, plus the core taxonomy-vs-object-type contract that R1 rested on. It fails against the pre-fix commit for each defect, so it is a real guard rather than a description of current behaviour.
...
|---|---|---|---|
| P67-J | Deleted symbols unreferenced (grep clean); `mullion_oembed_failure_count` autoload=no; `get_effective_campaign_level` uses `user_can($user_id,…)` | Full suite + PHPCS green; `Mullion_P52A_Permission_Matrix_Test`, `Mullion_Image_Optimizer_Test` green | ☐ |
...
| P67-H | Save with a dead webhook endpoint returns fast; delivery logged via cron with attempt 1 | `Mullion_P39IN1_Webhook_Test` + new cron-path test green | ☐ |
| P67-C | Single & batch 409 duplicate shape identical; `upload_font` uses the shared error map | `Mullion_P28N_Duplicate_Detection_Test` green | ☐ |
...
| P67-B | Identical query args + cache keys for identical inputs | `Mullion_P67B_Query_Builder_Test` + campaign-listing suites green | ☐ |
| P67-A | Out-of-range/invalid inputs clamp/fall-back identically; `sanitize_settings` ~61% smaller | `Mullion_Settings_Test` / `_Extended_Test` / `_Rest_Test` green (byte-for-byte) | ☐ |
| P67-I | Size sort orders by real file size both directions; all three write paths stamp `_mullion_filesize` | `Mullion_P67I_Filesize_Sort_Test` green | ☐ |
| P67-R | §4 R1–R4: priming warms post+term caches on the `fields => 'ids'` path; unreadable files stamp `0`; backfill bounded + resumes on cron; refused schedule delivers inline | `Mullion_P67R_Review_Fixes_Test` + `Mullion_Cron_Hooks_Test` green | ☐ |

**Automated baseline (must be green alongside manual QA):** full wp-env PHPUnit suite — **1274 tests, 13609 assertions, 0 failures, 2 pre-existing skips**, `composer lint:php` clean. Phase 67 adds 20 tests (9 for P67-B + 4 for P67-I + 7 for P67-R) and removes 2 (the dropped `Mullion_Image_Optimizer::get_stats()` tests) from the P66 landing baseline of 1255. See PHASE67_REPORT.md → each track's *Implementation* block for the exact per-track rationale and any line-number/scope corrections surfaced during execution, and its *PR Review & Fix Pass* section for the four post-landing fixes and the six areas that were challenged and cleared.
