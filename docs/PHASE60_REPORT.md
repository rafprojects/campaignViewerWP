...
| P60-C | Plugin Check + escaping/sanitization compliance pass | 🚧 Security core done (2026-07-04); WP.org-tier polish + lint annotations follow-on | Medium |
...
**Net result.** Plugin Check shipped-code **errors 27 → 15** after the 12 conformance fixes (unlink/parse_url/wp_count_terms all cleared); PHPUnit **1072/1072 green** post-fix. The remaining 15 shipped errors are all documented/justified: 4 `ExceptionNotEscaped` (trusted-path/numeric), 7 `NotPrepared` (verified prepared/internal custom-table), 2 `rmdir` (intentional uninstall FS), 1 `UnescapedDBParameter` (internal `$table`), 1 `outdated_tested_upto_header` (readme → P60-F). The ~300 shipped warnings are the accepted custom-table `$wpdb` architecture + the intentional enqueue/logging items above.

**Scope note.** P60-C's Problem/Fix framed the *security* surface (escaping, sanitization, nonce/capability) — that core is **done**: WPCS-security clear-cut items fixed, **dependency CVEs eliminated** (`composer audit` clean, lock pinned), **injection ruled out**, and the 12 low-risk WP-conformance fixes applied on top. Still open (deliberately not touched this session, per user direction): (1) the **14 WPCS-security false-positives** await sign-off before in-code `phpcs:ignore` annotation; (2) full **Plugin-Check-clean** (suppressing/annotating the accepted custom-table warning class + the readme "Tested up to" bump) is the **WP.org-listing gate**, not the Freemius-premium gate (Key Decision B) — a documented follow-on. The security-critical work is complete; what remains is annotation bookkeeping and WP.org-tier polish.
