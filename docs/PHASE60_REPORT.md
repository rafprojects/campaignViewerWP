...
| I-7b | Full fr/es translation sweep of all 2,426 keys + wp-env QA under a non-English locale | — | Deferred (user: lint-flip + English QA first) |
...
- **I-7b — deferred** (per user direction: lint-flip + English QA first, before investing in translation). The fr_FR + es_ES packs still cover only the ~249 original customer-facing keys; translating the ~2,150 new admin keys per language + wp-env QA under a locale is the outstanding follow-up.
