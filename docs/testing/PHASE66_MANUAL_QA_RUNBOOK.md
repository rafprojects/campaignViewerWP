...
|---|---|---|
| P66-A | `status` written at 7 sites, none recording *when* it changed | `Mullion_Campaign_Status::set()` writes status **and** `archived_at`/`restored_at` atomically; every site calls it |
...
| P66-D | Duplicate lost its space and its category/tag terms | Duplicator copies `_mullion_space_id` + both taxonomies |
| P66-E | Templates surfaced as draft campaigns in listings | `list_campaigns` + the wp-admin list table exclude `_mullion_is_template` |
...
**What & why.** Archiving or restoring a campaign now goes through one method, `Mullion_Campaign_Status::set($id, $status, $ctx)`, which writes the `status` meta **and** the matching timestamp in one place. `archived_at` is set when a campaign enters the archived state and cleared on restore (with `restored_at` written); a redundant re-archive does **not** reset `archived_at` (the purge clock must not restart). Audit/hook/cache side-effects are opt-in per caller, so external behavior (webhook volume, audit-log noise) is unchanged — the point of the track is only that the timestamp is now written *everywhere the status is*.
...
```bash
# Archive:
curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/campaigns/$CID/archive" | jq .
...
# Restore:
curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/campaigns/$CID/restore" | jq .
...
```bash
curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/campaigns/$CID/archive" >/dev/null
...
```bash
# Batch REST:
curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/campaigns/batch" \
...
# (restore, then) WP-CLI:
curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/campaigns/$CID/restore" >/dev/null
npx wp-env run cli wp mullion campaign archive $CID
npx wp-env run cli wp post meta get $CID archived_at    # → timestamp

# (restore, then) generic update endpoint with status=archived:
curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/campaigns/$CID/restore" >/dev/null
curl -s $AUTH -X PUT "$BASE/wp-json/mullion-gallery/v1/campaigns/$CID" \
...
npx wp-env run cli wp cron event run mullion_schedule_auto_archive
npx wp-env run cli wp post meta get $CID status         # → archived
...
**Regression checks.** `Mullion_P66A_Campaign_Status_Test` (14 tests) + the extended `Mullion_Auto_Archive_Cron_Test`. Existing archive/restore coverage (`Mullion_Campaign_Rest_Test`, `Mullion_CLI_Test`) stays green — audit/hook/cache behavior is deliberately unchanged.

**Pitfalls.**
- `archived_at`/`restored_at` are stored as **UTC** `Y-m-d H:i:s` (via `gmdate`), matching every other Mullion datetime. Don't compare against local-time strings.
- The generic **update** endpoint (`PUT /campaigns/{id}`) stamps `archived_at` when `status` flips to archived but deliberately fires no `mullion_campaign_archived` hook — it emits `campaign.updated`, exactly as before. That's intended, not a miss.

---

### P66-B — auto-purge keys off `archived_at`, not creation date

**What & why.** `Mullion_Maintenance::trash_archived_campaigns()` now selects archived campaigns whose **`archived_at`** is older than `archive_purge_days`, instead of `post_date_gmt`. A campaign created two years ago but archived yesterday is no longer trashed on the next cron run.
...
npx wp-env run cli wp eval '$s = get_option("mullion_settings", []); $s["archive_purge_days"] = 30; update_option("mullion_settings", $s);'

# A) created long ago, archived yesterday → must SURVIVE:
OLD=$(npx wp-env run cli wp post create --post_type=mullion_campaign --post_title='Old, just archived' \
...
NEW=$(npx wp-env run cli wp post create --post_type=mullion_campaign --post_title='New, long archived' \
...
npx wp-env run cli wp cron event run mullion_archive_cleanup
...
npx wp-env run cli wp option delete mullion_archived_at_backfilled
...
**Regression checks.** The rewritten `Mullion_Maintenance_Test` (creation-date-independent purge, plus the "no `archived_at` ⇒ skip" conservative case) and `Mullion_P66B_Archived_At_Backfill_Test` (DB-entry, most-recent-wins, legacy-meta, now-fallback, already-set-skip, non-archived-ignored).

**Pitfalls.**
- `archive_purge_days` is **0 by default** (feature off). If the cron does nothing, confirm you actually set it.
- The purge is two-phase: `mullion_archive_cleanup` *trashes*, then `mullion_trash_purge` permanently deletes after `archive_purge_grace_days`. This track only changed **phase 1**; phase 2 still keys off `post_modified_gmt` (trash time), which is correct.
...
**What & why.** `record_analytics_event()`, `Mullion_DB::sync_media_refs()`, and `Mullion_DB::insert_access_request()` now stamp the campaign's `space_id` at insert (same resolution as `insert_audit_entry`). A one-time backfill corrects historical rows. The user-visible payoff: a space-scoped analytics summary counts events instead of reporting zero.
...
SID=$(npx wp-env run cli wp eval 'echo Mullion_DB::insert_space(["slug"=>"qa-space","name"=>"QA","isolation_mode"=>"delegated"]);')
SCID=$(npx wp-env run cli wp post create --post_type=mullion_campaign --post_title='In QA Space' --post_status=publish --porcelain)
npx wp-env run cli wp post meta update $SCID status active
npx wp-env run cli wp post meta update $SCID _mullion_space_id $SID

# Record three views:
for i in 1 2 3; do
  curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/analytics/event" \
    -H 'Content-Type: application/json' -d "{\"campaign_id\":$SCID,\"event_type\":\"view\"}" >/dev/null
done

# Space-scoped summary must be non-zero:
curl -s $AUTH "$BASE/wp-json/mullion-gallery/v1/analytics/summary?space=$SID" | jq '{totalViews, uniqueVisitors}'
# → totalViews 3 (was 0 pre-fix)

# Confirm the raw row carries the space:
npx wp-env run cli wp db query "SELECT space_id FROM $(npx wp-env run cli wp eval 'echo Mullion_DB::get_analytics_table();') WHERE campaign_id=$SCID LIMIT 1"
```
...
**Backfill.** Historical `space_id = 0` rows are corrected by joining the campaign's `_mullion_space_id` postmeta. To re-observe:

```bash
npx wp-env run cli wp option delete mullion_scoped_space_id_backfilled
npx wp-env run cli wp eval 'delete_option("mullion_db_version"); Mullion_DB::maybe_upgrade();'
...
**Regression checks.** `Mullion_P66C_Scoped_Space_Id_Test` (three writers stamp; space-filtered summary counts; backfill corrects history). `Mullion_P28H_Analytics_Test` and `Mullion_DB_Test` stay green (default space_id `0` is unchanged for campaigns with no space).

**Pitfalls.**
- A campaign with no `_mullion_space_id` meta resolves to `space_id = 0` — that's correct (it's in the Default Space, whose scoped queries don't filter). Use a campaign explicitly assigned to a delegated space to see a non-zero stamp.
...
**What & why.** `Mullion_Campaign_Duplicator::duplicate()` now copies `_mullion_space_id` and the `mullion_campaign_category` / `mullion_campaign_tag` taxonomies, alongside the `mullion_company` term it already copied.

**Pre-fix behavior.** Neither `_mullion_space_id` nor the category/tag terms were copied — the duplicate silently fell back to the Default Space and lost its categorization.

```bash
# Source in a delegated space, with a category and a tag:
SID=$(npx wp-env run cli wp eval 'echo Mullion_DB::insert_space(["slug"=>"dup-space","name"=>"Dup","isolation_mode"=>"delegated"]);')
SRC=$(npx wp-env run cli wp post create --post_type=mullion_campaign --post_title='Dup Source' --post_status=publish --porcelain)
npx wp-env run cli wp post meta update $SRC _mullion_space_id $SID
CAT=$(npx wp-env run cli wp term create mullion_campaign_category 'QA Cat' --porcelain)
TAG=$(npx wp-env run cli wp term create mullion_campaign_tag 'QA Tag' --porcelain)
npx wp-env run cli wp post term add $SRC mullion_campaign_category $CAT
npx wp-env run cli wp post term add $SRC mullion_campaign_tag $TAG

# Duplicate via REST:
DUP=$(curl -s $AUTH -X POST "$BASE/wp-json/mullion-gallery/v1/campaigns/$SRC/duplicate" \
  -H 'Content-Type: application/json' -d '{"name":"Dup Copy"}' | jq -r '.id')

npx wp-env run cli wp post meta get $DUP _mullion_space_id                         # → $SID (not the default space)
npx wp-env run cli wp post term list $DUP mullion_campaign_category --field=name   # → QA Cat
npx wp-env run cli wp post term list $DUP mullion_campaign_tag --field=name        # → QA Tag
```

**Expected (pass).** The duplicate's `_mullion_space_id` equals `$SID`, and it carries both terms. **Why it proves the fix:** pre-fix, `_mullion_space_id` on the duplicate is the Default Space id (or empty) and the category/tag term lists are empty.

**Regression checks.** `Mullion_P66D_Duplicate_Space_Taxonomy_Test`.
...
**What & why.** `list_campaigns()` and the wp-admin Campaigns list table now exclude `mullion_campaign` posts carrying `_mullion_is_template`. Templates stay reachable through the dedicated templates endpoint.
...
NORM=$(npx wp-env run cli wp post create --post_type=mullion_campaign --post_title='Real Campaign' --post_status=publish --porcelain)
npx wp-env run cli wp post meta update $NORM status active
TPL=$(npx wp-env run cli wp post create --post_type=mullion_campaign --post_title='Saved Template' --post_status=publish --porcelain)
npx wp-env run cli wp post meta update $TPL _mullion_is_template 1

# REST listing must include NORM, exclude TPL:
curl -s $AUTH "$BASE/wp-json/mullion-gallery/v1/campaigns?include_archived=true&per_page=50" \
...
curl -s $AUTH "$BASE/wp-json/mullion-gallery/v1/campaign-templates" | jq '.[].id'   # → includes $TPL
```
...
**Pitfall.** The exclusion is a `_mullion_is_template NOT EXISTS` meta clause. If a template post is *missing* the flag (hand-created), it will still list — that's correct; it isn't actually a template.

---

### P66-F — uninstall removes everything (options, tables, dirs, indexes, cron)

**What & why.** Uninstalling with data-removal enabled now removes every `mullion_*` option (including `mullion_webhook_endpoints`, which holds **webhook secrets**), both renamed tables (`mullion_assets`, `mullion_space_library_assoc`), the `mullion-fonts/` and `mullion-exports/` upload dirs, the two custom indexes added to core WP tables, and all **10** cron hooks. `mullion-exports/` is removed **regardless** of the preserve-data setting. The cron-hook list is a single source of truth (`includes/mullion-cron-hooks.php`) shared by `mullion_deactivate()` and `uninstall.php`, so the two can't drift.
...
npx wp-env run cli wp db query "SELECT option_name FROM wp_options WHERE option_name LIKE 'mullion\_%' ORDER BY option_name" > /tmp/opts_before.txt
npx wp-env run cli wp db query "SHOW TABLES LIKE 'wp_mullion\_%'" > /tmp/tables_before.txt
npx wp-env run cli wp db query "SHOW INDEX FROM wp_postmeta WHERE Key_name='mullion_postmeta_postid_key'"
npx wp-env run cli wp db query "SHOW INDEX FROM wp_termmeta WHERE Key_name='mullion_termmeta_termid_key'"
npx wp-env run cli wp cron event list --fields=hook | grep mullion

# Make sure data removal is ON (preserve flag OFF):
npx wp-env run cli wp eval '$s=get_option("mullion_settings",[]); $s["preserve_data_on_uninstall"]=false; update_option("mullion_settings",$s);'

# Trigger the real uninstall routine:
npx wp-env run cli wp plugin uninstall mullion-gallery --deactivate

# AFTER — nothing mullion-* should remain:
npx wp-env run cli wp db query "SELECT option_name FROM wp_options WHERE option_name LIKE 'mullion\_%'"   # → empty
npx wp-env run cli wp db query "SELECT option_name FROM wp_options WHERE option_name LIKE 'mullion\_thumb\_%'"  # → empty
npx wp-env run cli wp db query "SHOW TABLES LIKE 'wp_mullion\_%'"                                          # → empty
npx wp-env run cli wp db query "SHOW INDEX FROM wp_postmeta WHERE Key_name='mullion_postmeta_postid_key'"  # → empty
npx wp-env run cli wp db query "SHOW INDEX FROM wp_termmeta WHERE Key_name='mullion_termmeta_termid_key'"  # → empty
ls "$(npx wp-env run cli wp eval 'echo wp_upload_dir()["basedir"];')" | grep mullion   # → no mullion-fonts / mullion-exports / mullion-thumbnails / mullion-overlays
```

**Expected (pass).** Every AFTER query is empty. **Why it proves the fix:** pre-fix, the options query still lists `mullion_webhook_endpoints`, `mullion_font_library`, `mullion_recent_logs`, the `mullion_thumb_%` rows, etc.; `SHOW TABLES` still lists `wp_mullion_assets` and `wp_mullion_space_library_assoc`; the two indexes remain; and `mullion-fonts/` / `mullion-exports/` still exist.

**`mullion-exports/` removed even when preserving data:**

```bash
# Fresh install, create an export dir, turn preserve-data ON, uninstall:
npx wp-env run cli wp eval 'wp_mkdir_p(wp_upload_dir()["basedir"]."/mullion-exports"); file_put_contents(wp_upload_dir()["basedir"]."/mullion-exports/x.zip","x");'
npx wp-env run cli wp eval '$s=get_option("mullion_settings",[]); $s["preserve_data_on_uninstall"]=true; update_option("mullion_settings",$s);'
npx wp-env run cli wp plugin uninstall mullion-gallery --deactivate
ls "$(npx wp-env run cli wp eval 'echo wp_upload_dir()["basedir"];')/mullion-exports" 2>&1   # → No such file or directory
# …but campaigns/options survive (preserve honored) — spot-check a mullion_campaign post still exists.
```

**Expected (pass).** `mullion-exports/` is gone even though preserve-data kept everything else. **Why it proves the fix:** Key Decision C — the 24h export-job TTL makes preserving ZIPs past uninstall backwards; the removal runs *before* the preserve-data early return.

**Cron-hook single-source-of-truth.** Verified automatically by `Mullion_Cron_Hooks_Test`: the canonical list matches every originating class constant (drift guard) and `mullion_deactivate()` clears the whole set. Manually: `wp cron event list | grep mullion` returns nothing after deactivation.

**Regression checks.** `Mullion_Cron_Hooks_Test`. Uninstall itself runs in an isolated process, so it is validated by the before/after diff above plus the cron-list unit test rather than a PHPUnit case that boots the uninstall file.

**Pitfalls.**
- `wp plugin uninstall` only runs `uninstall.php` if the plugin is **not** active — the `--deactivate` flag handles that.
- The `LIKE 'mullion\_%'` escaping matters: an unescaped `_` is a single-char wildcard. The AFTER queries above escape it, matching the deletion queries.
- Migrators must move real ZIPs out of `uploads/mullion-exports/` **before** uninstalling — this is now documented in [INSTALL_AND_TROUBLESHOOTING.md](guides/INSTALL_AND_TROUBLESHOOTING.md#uninstalling).
...
|---|---|---|---|
| P66-A | `archived_at`/`restored_at` stamped through every archive/restore entry point; clock not reset on re-archive | Audit/hook/cache behavior unchanged; `Mullion_P66A_Campaign_Status_Test` (14) + auto-archive cron green | ☐ |
| P66-B | Old-but-recently-archived campaign survives purge; new-but-long-archived is trashed | `Mullion_Maintenance_Test` + `Mullion_P66B_Archived_At_Backfill_Test` green | ☐ |
| P66-C | Space-filtered analytics summary is non-zero; 3 writers stamp `space_id`; backfill corrects history | `Mullion_P66C_Scoped_Space_Id_Test`; P28H/DB tests green | ☐ |
| P66-D | Duplicate keeps its space + category/tag terms | `Mullion_P66D_Duplicate_Space_Taxonomy_Test` green | ☐ |
| P66-E | Templates absent from `campaigns.list` + wp-admin table; still in templates API | `Mullion_P66E_Template_Listing_Test`; P28O green | ☐ |
| P66-F | Before/after diff: zero `mullion_*` options/tables/dirs/indexes; all 10 cron hooks cleared; `mullion-exports/` gone even when preserving | `Mullion_Cron_Hooks_Test` green | ☐ |
