...
| A **production** front-end build (`npm run build`), served through the shortcode | **Critical for P69-B** — `isMullionDebugEnabled()` short-circuits to `true` whenever `import.meta.env.DEV` is set, so a dev build always shows debug markers regardless of the PHP default. The default-flip is only observable against a prod build. |
...
**Personas / auth.** Same RBAC model as Phase 63–68 — see §2 of [PHASE63_MANUAL_QA_RUNBOOK.md](PHASE63_MANUAL_QA_RUNBOOK.md) for creating a System Admin and a `mullion_editor`.
...
| P69-B | `debug_component_markers` default flipped `true → false` in both `class-mullion-settings-registry.php` (canonical) and `class-mullion-embed.php` (defensive fallback) | **Yes** — a fresh install's public gallery no longer stamps `data-mullion-component` / `data-mullion-slot` attributes (prod build) |
| P69-C | `parseNodeConfig` (extracted from `src/main.tsx` into `src/mountConfig.ts`) now validates `data-mullion-config` through a zod schema (allowlist + type-check), matching `parseProps`'s `ALLOWED_PROPS` treatment | **Yes** — a crafted config payload with extra/wrong-typed keys is stripped/coerced instead of passed through |
| P69-D | `ErrorBoundary.tsx` shows generic translated copy to the public; the raw `error.message` only when the viewer is an admin **or** `mullion_debug` is set. Also adds a public-facing boundary around `<App>` in `main.tsx` (the gallery had none) | **Yes** — a thrown error renders differently for a public visitor vs. an admin/debug session |
| P69-E | `docs/FUTURE_TASKS.md`'s "JWT In-Memory Token Auth" item now cross-references the `mullion_permissions` cache/TTL gap and no longer claims the provider is commented out | No — documentation only |
...
**Regression checks.** None — no code changed. The existing `loadGoogleFont.test.ts` and `Mullion_Settings_Typography` coverage remain green because they were not touched.
...
**What & why.** `debug_component_markers` defaulted to `true`, so every production install stamped `data-mullion-component` / `data-mullion-slot` attributes onto every tile, row, and panel of every public gallery — payload and DOM-size overhead for a debugging aid nobody opted into. P69-B flips the default to `false` in **both** places it was hardcoded:
- `class-mullion-settings-registry.php:30` — the canonical `self::$defaults` array.
- `class-mullion-embed.php` `page_config_js()` — a defensive `isset(...) ? ... : true` fallback that only fires if `Mullion_Settings` fails to load at all (since `get_settings()` always merges the registry default). Flipped to `false` for consistency so no code path still answers "true".

The admin toggle (Settings → Advanced) and the `mullion_debug_component_markers` filter remain the explicit opt-in. The `Mullion_DEBUG_COMPONENT_MARKERS` constant override is untouched (intentional dev/test escape hatch).

**Pre-fix behavior.** A fresh install with no settings customization rendered `debugComponentMarkers: true` into `window.__MULLION_CONFIG__`, and every gallery component carried `data-mullion-*` debug attributes.
...
npx @wordpress/env run cli wp option delete mullion_settings   # or a fresh install

# 2) Confirm the emitted config now says false:
npx @wordpress/env run cli wp eval '
  $js = Mullion_Embed::page_config_js();
...
```
DevTools → Elements → search for "data-mullion-component"
...
```bash
npx @wordpress/env run cli wp eval 'update_option("mullion_settings", ["debug_component_markers" => true]);'
# Reload the public gallery → data-mullion-component / data-mullion-slot reappear.
```
...
**Regression checks.**
- **New:** `Mullion_Settings_Extended_Test::test_get_defaults_debug_component_markers_is_false` asserts `get_defaults()['debug_component_markers'] === false`. (No prior test locked in this default — see PHASE69_REPORT.md Planning Refinement Pass #2.)
- **Unchanged:** `Mullion_Embed_Test::test_render_shortcode_reflects_debug_component_markers_setting` (sets it to `false`, asserts `"debugComponentMarkers":false`), `Mullion_Settings_Test::test_sanitize_settings_persists_false_for_classic_checkbox_hidden_inputs`, and `Mullion_Settings_Rest_Test` all pass unmodified — none asserted the default was `true`.

**Pitfall.** `isMullionDebugEnabled()` (`src/utils/mullionDebug.ts`) is `enabled ?? (import.meta.env.DEV || (window.__MULLION_CONFIG__?.debugComponentMarkers ?? false))` — it forces markers **on in any dev build** regardless of the PHP default. If you validate against `npm run dev` you will *always* see the attributes and wrongly conclude the fix failed. Test against `npm run build` output only.

---

### P69-E — JWT permissions-cache gap cross-referenced (tracking only)

**What & why.** `WpJwtProvider.getPermissions()` returns the cached `mullion_permissions` `localStorage` entry with no TTL — cleared only on logout — so a revoked grant lingers in the client UI until logout (display-only; the server still enforces). The provider is opt-in (`MULLION_ENABLE_JWT_AUTH`, off by default), so this is not scheduled implementation work this phase. P69-E's only action is a documentation edit: `docs/FUTURE_TASKS.md`'s "JWT In-Memory Token Auth" item now cross-references this gap (the prior record, `REACT_REVIEW_FINDINGS.md`, has been archived), and the stale claim that the provider is "commented out" is corrected (it is live, flag-gated `.ts` code).
...
- a bullet now describes the `mullion_permissions` no-TTL cache staleness and points back to PHASE69_REPORT.md § P69-E;
...
**What & why.** At mount, PHP stamps two attributes onto each host: `data-mullion-props` and `data-mullion-config`. `parseProps` filtered `data-mullion-props` through the `ALLOWED_PROPS` allowlist, but `parseNodeConfig` cast `data-mullion-config` to `NodeConfig` via a bare `as NodeConfig` assertion — a compile-time-only claim with **no runtime allowlist or type-check**. The attribute is PHP-generated today (low risk), but the asymmetry is the kind that erodes: anything else that can set that attribute (a page-builder plugin storing raw HTML, an XSS elsewhere) got arbitrary keys/types into the mount config. P69-C gives `data-mullion-config` the same treatment via a small zod schema (mirroring the idioms already in `src/types/settingsSchemas.ts`). The parsing was extracted into `src/mountConfig.ts` so it can be unit-tested without importing `main.tsx`'s side effects.
...
**Observable effect: malformed config is stripped/coerced.** This is primarily proven by the unit test; the live effect is only visible if you deliberately craft a bad `data-mullion-config`.
...
```js
// DevTools console, on a page with a Mullion mount host:
const host = document.querySelector('[data-mullion-config]');
host.setAttribute('data-mullion-config', JSON.stringify({ spaceId: 3, evil: 'x', theme: 42 }));
// Re-mount (reload). The app receives { spaceId: 3 } — 'evil' stripped, wrong-typed 'theme' dropped.
```

**Why it proves the fix.** Pre-fix, `evil` and the numeric `theme` would flow into the component tree verbatim; post-fix the schema strips the unknown key and drops the wrong-typed one, so only allowlisted, correctly-typed fields reach the app — matching `parseProps`'s guarantee for `data-mullion-props`.
...
- **Unchanged:** the PHP side that *generates* `data-mullion-config` is untouched, and `Mullion_Embed_Test`'s `data-mullion-config` shape assertions remain green — legitimate PHP payloads (numeric `spaceId`, string `theme`, boolean toggles) all satisfy the schema, so there is no behavior change for real traffic.
...
Covers: public viewer (no `isAdmin`, no debug) sees generic copy and **not** the raw message; admin viewer (`isAdmin`) sees the raw message; `mullion_debug=1` reveals the raw message even for a non-admin.
...
- **As an admin**, or with `localStorage.setItem('mullion_debug', '1')` then reload: the same error now shows the raw `error.message` for troubleshooting.
...
| P69-B | Fresh-install **prod build** public gallery has no `data-mullion-*` attributes; toggling the setting on restores them; emitted config is `debugComponentMarkers:false` | `Mullion_Settings_Extended_Test` new default test + `Mullion_Embed_Test` / `Mullion_Settings_Rest_Test` green | ☐ |
| P69-C | Crafted `data-mullion-config` with extra/wrong-typed keys is stripped/dropped; legitimate PHP payloads unchanged | `Mullion_Embed_Test` config-shape assertions green; new `src/mountConfig.test.ts` green | ☐ |
| P69-D | Public/non-admin visitor sees generic copy on an error boundary; admin or `mullion_debug` session sees the raw message; public gallery now has a boundary | Updated `ErrorBoundary` unit tests (public/admin/debug) green | ☐ |
| P69-E | `FUTURE_TASKS.md` cross-references the `mullion_permissions` TTL gap and no longer claims the provider is commented out | None — documentation only | ☐ |
...
| PHPUnit settings/embed slice (`Mullion_Settings_Test`, `Mullion_Settings_Extended_Test`, `Mullion_Settings_Rest_Test`, `Mullion_Embed_Test`) | OK (22 tests, 154 assertions) |
