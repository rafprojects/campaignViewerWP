...
### P72-E — Split AdminPanel tabs into re-render-isolated child components (**all three landed**)

**What & why.** `AdminPanel.tsx` (~925 lines) held every tab's selection state inline, so changing (say) the audit-tab campaign selector set state on `AdminPanel` and re-rendered the *entire* panel — the campaigns table, the media toolbar, every memo. Option (b) of the track splits each data tab into a child component that owns its own selection state, so a selection change re-renders only that child. Delivered **incrementally, one panel per commit**: **AuditPanel** (least-coupled, first), then **AccessPanel** (the heavy one), then **MediaPanel** (last, smallest). `AdminPanel` dropped to ~800 lines and owns none of the three tabs' selection state anymore.
...
**What moved.** `AuditPanel.tsx` owns `auditCampaignId` + `auditFilters`, calls `useAuditEntries`/`useAuditRows`, runs the default-to-first-campaign effect, and runs the once-per-mount audit prefetch. `AccessPanel.tsx` owns the four access atoms (`accessViewMode`/`accessCampaignId`/`selectedCompanyId`/`showExpiredGrants`), the `useAccessGrants` + `useCompanies` hooks, the `useAdminAccessState` form-state hook (+ its `mutate*Wrapped` callbacks), the `companySelectData`/`selectedCampaign`/`selectedCompany` memos, `useAccessRows`, both default-select effects, the once-per-mount access prefetch — **and the two access modals** (`ArchiveCompanyModal`, `QuickAddUserModal`), which are driven entirely by `accessState` and so had to move with it (leaving them at the root would keep `accessState` lifted and defeat the isolation). `MediaPanel.tsx` owns `mediaCampaignId` + the `rescanAllLoading` flag, the media default-select effect, the once-per-mount media prefetch, the full media toolbar (CampaignSelector + system-admin ZIP export/import + Rescan-All), and the lazy `MediaTab`; **`addMediaCampaign` stayed in `AdminPanel`** (a Campaigns-tab row action whose modal renders at root — not media-local). `AdminPanel` renders each as `<AuditPanel key={selectedSpaceId} active={activeTab === 'audit'} … />` / `<AccessPanel … active={activeTab === 'access'} … />` / `<MediaPanel … active={activeTab === 'media'} … />`. The `key={selectedSpaceId}` remounts each child on space change — that is what now resets its selection; the shared `selectedSpaceId` reset effect was **removed entirely** once all three tabs self-reset.
...
**Verification (primary — automated).**
```bash
npx vitest run src/components/Admin/AuditPanel.test.tsx    # 3 tests: default-select+fetch, inactive no-fetch, parent-isolation
npx vitest run src/components/Admin/AccessPanel.test.tsx   # 3 tests: same shape, access-grants fetch
npx vitest run src/components/Admin/MediaPanel.test.tsx    # 3 tests: same shape, selector reflects selection
npx vitest run src/components/Admin/AdminPanel.test.tsx    # existing tests, unchanged, green
npx tsc -b
npx eslint src/components/Admin/AuditPanel.tsx src/components/Admin/AccessPanel.tsx src/components/Admin/MediaPanel.tsx src/components/Admin/AdminPanel.tsx
```
The **isolation test is the "only meaningful under (b)" assertion**: a stable-prop parent harness renders the child; the child's mount-time default-select drives its data fetch / selection (proof its own state changed), yet the parent's render count stays exactly `1`. Under option (a) — hook extraction with the state still living in `AdminPanel` — that default-select would have bumped the parent. The fetch-gating test (`active={false}` ⇒ no `/audit` or `/access` request, no media auto-select) proves the mount-stays-mounted correction didn't leak fetches.

**Live check — the isolation win (the point of this refactor; recommended, and the only manual check specific to it).** The behaviour changes are invisible in normal use — what actually changed is *which* React tree re-renders. Verify it with the **React DevTools Profiler** (or "Highlight updates when components render" in the DevTools settings):
1. Open the admin panel, go to a data tab, start a Profiler recording (or enable highlight-updates).
2. Change that tab's selector — e.g. pick a different campaign in the Audit tab, toggle View By in Access, or switch campaigns in Media.
3. **Confirm only the child panel (`AuditPanel`/`AccessPanel`/`MediaPanel`) and its subtree re-render** — the `AdminPanel` header, the Campaigns table, and the *other* tabs' panels should **not** flash / appear in the commit. Before P72-E, that same interaction re-rendered all of `AdminPanel`. (Highlight-updates makes this immediate: only the changed panel should get an outline.)

**Live check — nothing regressed (recommended).** *Audit:* Campaign Activity tab → first campaign auto-selected + activity loads; switch campaigns → table updates; exercise the **filter fields** (from/to date, action, scope, severity) → the table re-queries and the CSV/ZIP export reflect the current filters; change the space dropdown → audit campaign resets to the new space's first. *Access:* Access tab → first campaign auto-selected + grants load; toggle View By (system admin) between Campaign / Company / All → each defaults its selection and loads; toggle **"show expired grants"** → expired rows appear/disappear; grant/revoke a user; open "Quick add user" and archive-company flows (both modals now live inside AccessPanel) → confirm they still open, submit, and notify; change the space dropdown → access selection resets. *Media:* Media tab → first campaign auto-selected + its media grid loads; switch campaigns → grid updates; as a system admin confirm Export ZIP / Import ZIP work and **Rescan All** runs and shows its success/"all correct" (or error) toast; confirm the per-row "Add media" action on the **Campaigns** tab still opens its upload modal (that stayed in AdminPanel); change the space dropdown → media selection resets. *All tabs:* switch away and back → no double-fetch, and the prior selection is **retained** (confirms the `<Activity mode="hidden">` state-preservation and the once-per-mount prefetch guard).

**Regression checks.** Existing `AdminPanel` tests pass unmodified; full front-end suite green (**255 files / 3775 tests**, +9). The diff is contained to `AdminPanel.tsx` (state/hook/effect/modal/toolbar removals + three child renders) and the new `AuditPanel.tsx` / `AccessPanel.tsx` / `MediaPanel.tsx` — no shared hook, query, or moved modal/toolbar code changed.
