...
**Live check (recommended).** *Audit:* Campaign Activity tab → first campaign auto-selected + activity loads; switch campaigns → table updates; change the space dropdown → audit campaign resets to the new space's first; CSV/ZIP export still download for the current selection + filters. *Access:* Access tab → first campaign auto-selected + grants load; toggle View By (system admin) between Campaign / Company / All → each defaults its selection and loads; grant/revoke a user; open "Quick add user" and archive-company flows (both modals now live inside AccessPanel) → confirm they still open, submit, and notify; change the space dropdown → access selection resets. *Media:* Media tab → first campaign auto-selected + its media grid loads; switch campaigns → grid updates; as a system admin confirm Export ZIP / Import ZIP / Rescan All still work (they live in MediaPanel now); confirm the per-row "Add media" action on the **Campaigns** tab still opens its upload modal (that stayed in AdminPanel); change the space dropdown → media selection resets. Switch tabs away and back → no double-fetch, selection retained.
