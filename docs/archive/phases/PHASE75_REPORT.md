...
[P53-D](PHASE53_REPORT.md#track-p53-d---access-grant-model-simplification-decided-2026-06-15-viewer-only) reduced every grant endpoint's `access_level` enum to `['viewer']`, because editing/managing now comes from the `mullion_editor` role plus space access rather than from a per-grant level:
