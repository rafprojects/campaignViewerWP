...
- **`CampaignCardSettingsSection.tsx` (43/45 findings, the real bug):** confirmed `ResetLink` was defined inside the parent's render body, closing over `isDesktop`, `hasOverride`, `clearField`, `clearDimField`, `t`. Hoisted it to module scope as `ResetLink({ fieldKey, unitKey, hasOverride, onReset })`. Two things simplified the prop surface below the 4 originally-closed-over values:
  ...
  - `t()` didn't need to be threaded through — `ResetLink` originally called its own `useTranslation('wpsg')` directly (same namespace, standard i18next usage) rather than receiving the parent's translation function as a prop. **Superseded during the PR review pass** (see [PR Review](#pr-review-2026-07-25) below): 43 independent `useTranslation` subscriptions for one invariant string was real per-render overhead, so `ResetLink` now takes the already-resolved `label` string as a prop instead.
...
## PR Review (2026-07-25)

Self-reviewed the full branch diff (`main...HEAD`, all 6 commits, 24 files) as if reviewing an incoming PR — validated every implementation claim in this report against the actual code (not just re-reading the prose), then ran a 4-agent `/simplify` pass (reuse / simplification / efficiency / altitude) and a `/code-review` pass over the same diff. No GitHub PR exists for this branch yet, so there were no review-tool threads to fetch — this section *is* the review record.

### Validation of existing claims

Spot-checked the report's more load-bearing assertions directly against source rather than trusting the prose:

- `hasOverride(key)` really does `if (isDesktop) return false;` first ([CampaignCardSettingsSection.tsx:75](../src/components/Settings/CampaignCardSettingsSection.tsx#L75)) — confirms P73-D's claim that dropping the separate `isDesktop` check from the hoisted `ResetLink` was behavior-preserving.
- All 43 `<ResetLink>` call sites carry the same `hasOverride`/`onReset` props (`grep -c` matched call-site count exactly) — the P73-D `sed` substitution left nothing half-migrated.
- `templateSlotsRef` is fully gone from `LayoutCanvas.tsx` — no dangling references from the "dead code, deleted" claim in P73-E.
- `useTheme.test.ts` still calls `renderHook(() => useTheme())` directly (bypassing the shared `Providers` wrapper) — P73-B's claim that the outside-provider warning path is still independently tested, not just silenced, holds.
- `useElementSize`/`useMergedRef` both exist in the installed `@mantine/hooks@9.3.1` and are correctly exported/used — no phantom API.
- `docs/OUTSTANDING_VULNERABILITIES.md`'s `serve` claims cross-checked live: `playwright.visual.config.ts:42` does run `npx serve storybook-static`, and a fresh `npm audit` still reports the exact `brace-expansion`/`minimatch`/`serve`/`serve-handler` chain — the doc is accurate today, not just at write time.
- `npm run lint`, `npm run build`, and `npm run test:silent` (255 files / 3775 tests) all re-run clean from a fresh checkout, independent of the implementation session's own verification.

### Findings and dispositions

| # | Source | Finding | Disposition |
|---|---|---|---|
| 1 | `/code-review` | [CampaignCardSettingsSection.tsx](../src/components/Settings/CampaignCardSettingsSection.tsx) — hoisted `ResetLink` called its own `useTranslation('wpsg')` at all 43 render call sites for one invariant string, instead of the parent's already-in-scope `t`. | **Fixed.** `ResetLink` now takes a `label: string` prop; the parent resolves `resetLinkLabel = t('set_card_reset_inherited', ...)` once and passes it to all 43 call sites (bulk `sed`, then `grep -c` verified 43/43 landed). Removes 42 redundant `useTranslation` context subscriptions per render for zero behavior change. |
| 2 | `/code-review` | [LayoutBuilderCanvasPanel.tsx:387](../src/components/Admin/LayoutBuilder/LayoutBuilderCanvasPanel.tsx#L387) — `canvasAreaWidth \|\| activePresetWidth`: `useElementSize`'s `width` starts at `0` and only updates once the `ResizeObserver`'s first callback fires asynchronously, so the device-preview frame can render briefly unclamped (full preset width) before that first callback lands — plausible on opening the builder already in preview mode with a persisted device preset (`previewPreset`/`showPreviewFrame` are read from `localStorage` on mount). | **Investigated, left as documented trade-off, not fixed.** The obvious fix — falling back to `canvasAreaRef.current?.clientWidth` — reads a ref during render, which is *exactly* the `react-hooks/refs` anti-pattern P73-E's `useElementSize` swap exists to eliminate; a first attempt at that fix was caught immediately by `npm run lint` re-failing on the rule this track adopted. Trading a real staleness bug (the original P73-E fix) for a one-frame cosmetic flash (self-correcting well under 16ms) isn't a good trade. Documented in a code comment ([LayoutBuilderCanvasPanel.tsx:193-208](../src/components/Admin/LayoutBuilder/LayoutBuilderCanvasPanel.tsx#L193-L208)) rather than silently left unexplained. |
| 3 | `/code-review` | `docs/PHASE73_REPORT.md`'s P73-D Implementation Notes claimed `ResetLink` "calls its own `useTranslation('wpsg')` directly" — true when written, now stale after finding #1's fix. | **Fixed.** Updated the P73-D note in place to point at this section instead of describing the superseded design. |
| 4 | Manual read-through | Execution Priority list had a stale duplicate line — `6. ~~P73-F~~ — done.` immediately followed by the original un-struck `6. P73-F — medium, triage-first...` planning line that should have been removed when the track was marked done. | **Fixed.** Removed the leftover planning line. |
| 5 | `/simplify` (efficiency) | Same as #1 above — independently caught by both the efficiency agent and `/code-review`. | Fixed once, counted under #1. |
| 6 | `/simplify` (reuse) | [LayoutBuilderCanvasPanel.tsx:209-210](../src/components/Admin/LayoutBuilder/LayoutBuilderCanvasPanel.tsx#L209-L210) — the repo already has a ref-in/width-out hook with matching semantics, `useContainerWidth` ([Adapters/_shared/useContainerWidth.ts](../src/components/Galleries/Adapters/_shared/useContainerWidth.ts)), which takes a *consumer-supplied* ref (so no `useMergedRef` needed) and even seeds a synchronous `clientWidth` read before the `ResizeObserver` fires. | **Considered, not applied.** `useContainerWidth` is scoped to `Adapters/_shared/` (gallery-adapter family only, per its own docstring — "extracted verbatim from the Diamond/Hexagon adapters") and has exactly one consumer today (`ClippedTileGridGallery.tsx`); there's no existing precedent for importing across that boundary into `Admin/LayoutBuilder`. The clean move would be promoting it to `packages/shared-utils` first, which is a real (if small) refactor of a working, tested utility's location plus updating its existing call site — out of scope for a lint-adoption PR review. Left as Mantine's `useElementSize`, a well-tested library primitive, rather than reaching across a feature boundary for this pass. Worth doing as its own small follow-up if `useContainerWidth`-shaped needs come up again outside the adapter family. |
| 7 | `/simplify` (simplification) | [CampaignCardSettingsSection.tsx](../src/components/Settings/CampaignCardSettingsSection.tsx) — hoisting `ResetLink` turned ~35+ call sites from 2 props to 4-5 identical props (`hasOverride`, `onReset`, now also `label`) at every site; a `ResetLinkContext.Provider` would restore the shorter call sites. | **Rejected.** Explicit props are more traceable/testable than an ad-hoc context for a component with only 3 stable, parent-scoped values — introducing a context provider here trades one boilerplate cost (repeated props) for a different one (indirection through context, harder to grep call sites for what a given `ResetLink` actually depends on). Not worth it for this component's shape. |
| 8 | `/simplify` (altitude) | Soft observation only: the 3 `preserve-manual-memoization` suppressions (P73-C) use near-identical "compiler can't prove immutability, we don't run the compiler" reasoning to the wholesale non-adoption of `set-state-in-effect` (P73-F) — a threshold call about when to suppress per-site vs. reject the rule outright. | **No action** — already an explicit, reasoned trade-off in the original tracks (3 sites is cheap to suppress individually; 42 near-identical sites is not), not a new finding. |

All other findings from the review agents (checked: no leftover manual `useRef`+assignment mirror patterns anywhere in the repo outside what P73-E already migrated; no `useCallback`/`useEffect` hot-path regressions from the new `useLatestRef`-derived dependency-array entries, since `useRef`-backed identity is stable; no suppression scoped too broadly) came back clean — reported as "nothing found," not included above.

### Verification

- `npm run lint`, `npm run build`, `npm run test:silent` (255 files / 3775 tests) all re-confirmed clean after every fix in this pass, not just once at the end.
- No UI-visible behavior changed by any fix in this section — #1 and #3 are render-cost/documentation-only, #4 is a doc-only correction; #2 was investigated and deliberately left as-is.
