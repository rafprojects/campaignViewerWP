...
The plugin's designer, working from [docs/design/DESIGN_BRIEF.md](design/DESIGN_BRIEF.md), returned two things: a product name — **Mullion** (slug `mullion-gallery`) — and a new brand palette, **Rig Cyan**. Both are adopted here.
...
5. **Success.** Every `wpsg`/`WPSG`/`WP Super Gallery`/`wp-super-gallery` identifier in shipped code, config, and top-level docs reads as `mullion`/`Mullion`/`Mullion`/`mullion-gallery` instead (historical phase reports under `docs/archive/` excepted, left as an accurate record of what shipped under the old name; the P74-K license-test negative assertion is the one remaining old-slug grep in plugin PHP); the plugin loads cleanly under its new folder/slug/text-domain in wp-env; local wp-env data is reseeded rather than migrated (Decision H); and the new default theme (Rig Cyan) passes the same automated WCAG AA gate every other theme does. Rig Cyan's `primaryShade` is **not** a Phase 74 closer — it is set in P75-F against the OKLCH ramp (Decision I).
...
| E | CPT slug `wpsg_campaign` and the 276 `wpsg_*` option keys: rename and accept data loss on existing dev/staging installs, or migrate | **Migrate (original).** A one-time routine, self-disabling after it runs. Revisited as **H** once the rename had landed: keeping a shipped FROM-map of `wpsg_*` is not the same as a public alias, but it still leaves the old prefix in shipped PHP forever. |
...
| H | Keep the rebrand migrator / uninstall old-name sweeps / P52 `wpsg_admin` leftover path now that the rename has landed, or drop them | **Drop.** Same pre-launch call as **D**. Those `wpsg_*` strings are not public aliases — they are the FROM side of a rewrite map, so a mechanical rename would blind the migrator rather than finish the rebrand. The way to stop shipping the old prefix is to delete the machinery. Local wp-env campaign data is reseedable; there are no public installs. P74-Q. |
| I | Keep Rig Cyan's `primaryShade` as a P74-N closer (blocked on P75-F), or move that sub-item to P75-F so Phase 74 can close | **Move to P75-F.** `primaryShade` is a ramp *position*, not a hex. The current HSL generator resolves the spec's criterion at index 7; P75-F's OKLCH generator will resolve a different index. Setting a number in P74-N would bake in a value F immediately invalidates. P74-N ships the palette with no `primaryShade` and a `_primaryShade` note; P75-F sets the real index in the same commit as the ramp swap (alongside re-deriving the other 16 themes). |
...
8. **P74-Q** — leftover live identifiers the A–P sweeps missed, plus drop the rebrand migrator (Decision H). Lands before M so the docs sweep does not describe a migrator that no longer exists.
9. **P74-M** — documentation, last, so it describes the finished state rather than an in-progress one.
10. **P74-N** — independent of the identifier-rename tracks. Palette, schema fields, contrast gate, and snapshot rebaseline are the closer; `primaryShade` is P75-F (Decision I), not a Phase 74 hold.
11. **P74-O** — after P74-N's palette values are final. Does not wait on `primaryShade` / P75-F.
...
- `readme.txt`: `=== WP Super Gallery ===` → `=== Mullion ===`, the intro sentence, "WP Super Gallery Pro" → "Mullion Pro", and the install-instructions folder name (now literally correct post-P74-A: `wp-super-gallery` → `mullion-gallery`). **Deliberately left unchanged at the time:** `Contributors: wpsupergallery` (a WordPress.org account handle, not a product identifier) and "Navigate to **WP Super Gallery** in the admin menu..." — the admin-menu label had not moved yet. P74-E later set CPT `menu_name` to **Mullion**, so that install step is now wrong. Punch-list for **P74-Q**. Changelog history lines (`wp wpsg`, `@wpsg`) stay as an accurate record of what those versions shipped.
...
- **Migrator FROM-map revisited as P74-Q / Decision H.** The source strings in this class stayed `wpsg_*` so the rewrite could find pre-rename rows. That is not a public alias, but it still ships the old prefix. Q drops the class rather than renaming those keys.
...
- **Held back:** REST `/wp-super-gallery/v1/`, Freemius slug (P74-K), `wp-super-gallery-app` script handle, `docs/**` (P74-M). Uninstall old-name dual-lookups (`wpsg_campaign`, `wpsg_*` options/tables/dirs/roles) stayed so a delete-before-migrate left no orphans — **P74-Q** drops them with the migrator (Decision H).
...
- Manual code review — this function can't be meaningfully exercised without real Freemius credentials, consistent with how it's validated today. **Also met via source-level PHPUnit:** `Mullion_License_Test::test_freemius_bootstrap_slug_is_mullion_gallery` asserts both `fs_dynamic_init` slug defaults are `'mullion-gallery'` and that `'slug' => 'wp-super-gallery'` is absent from the bootstrap file. Existing `test_is_sdk_active_false_without_credentials` still covers the no-op path.
- Focused `Mullion_License_Test`: 14 tests, 26 assertions (was 13 / 23). Full PHPUnit: 1,314 tests, 13,741 assertions, 2 skipped, 0 failures (was 1,313 / 13,738).
...
- **The function rename was already done.** The Problem/Fix text still described updating the slugs "as part of the same edit that renames `wpsg_fs()` → `mullion_fs()`". P74-H already renamed the function, the `mullion_freemius_config` filter, and `mullion_fs_loaded`. This track is the two remaining `fs_dynamic_init` literals: product `'slug'` and `'menu' => ['slug']`, both now `'mullion-gallery'`.
- **Did not retarget `menu.slug` at a live admin page.** No WP admin page is registered as `mullion-gallery` (pages are `mullion-settings` / `mullion-assets` / `mullion-spaces` under the CPT parent). That mismatch already existed under `wp-super-gallery`. NOTE(M2) / P75-A own `first-path`, `premium_slug`, `is_premium`, `has_premium_version`, and `is_org_compliant` once the dashboard product exists. Both slug args match Key Decision C (one product slug).
- **Did not extract `mullion_freemius_init_args()`.** That is P75-A. `mullion_fs()` still no-ops with empty credentials (zero network calls). Added `Mullion_License_Test::test_freemius_bootstrap_slug_is_mullion_gallery` as a source-level gate because the SDK path cannot run in CI.
- **Folded a P74-C leftover:** `phpcs.xml` `<config name="text_domain">` was still `wp-super-gallery`, which would make WordPress.WP.I18n mis-sniff every `'mullion-gallery'` call. Ruleset display name left as prose for P74-M.
- **Held back on purpose:** REST namespace `/wp-super-gallery/v1/` (85 `register_rest_route` calls, 502 PHPUnit hits, 281 src hits — unclaimed, atomic, changing one side 404s the app); script handle `wp-super-gallery-app` / fallback `wp-super-gallery.js`; `docs/**` (P74-M); i18n GitHub URI msgids (P74-C `make-pot` backlog).
...
- **Missed, punch-list for P74-Q:** the "Bump version" step in `.github/workflows/release.yml` still `sed`s `define('WPSG_VERSION'` and greps `WPSG_VERSION`. P74-G renamed the constant to `MULLION_VERSION`; this track updated ZIP/`SLUG` in the same file and did not retarget the bump. A real release would bump the plugin header and leave the runtime constant at `0.90.0`.
...
- **Held back:** `docs/**` guides/runbooks/CHANGELOG/CONTRIBUTING (P74-M, except CONTRIBUTING path literals which **P74-Q** takes as tooling); `languages/` GitHub-URI msgids (P74-C `make-pot` backlog); phpcs ruleset display name `WP Super Gallery — Security` (prose — **P74-Q**); `.wordpress-org/*`. Gitignored built `assets/` copies were touched by the script but are not committed.

---

## Track P74-Q - Leftover live identifiers + drop the rebrand migrator

### Problem

A post-P audit of the Done tracks found live leftovers that A–P never claimed, plus a larger question about every remaining `wpsg_*` in shipped PHP.

**Missed live identifiers (not docs):**

1. **P74-L/G.** `.github/workflows/release.yml` still `sed`s `define('WPSG_VERSION', …)` and greps `WPSG_VERSION`. The constant is `MULLION_VERSION`. A release would bump the plugin header / `package.json` and leave the runtime constant at `0.90.0`.
2. **P74-H/J.** `wp_localize_script(…, 'wpsgSettingsAuthTest', …)` and `includes/settings/assets/settings-auth-test.js` (`window.wpsgSettingsAuthTest`, `dataset.wpsgBound`). The AJAX action was renamed to `mullion_test_auth`; the localize object was not. Settings → Test Auth still works because PHP and JS match each other.
3. **Unclaimed PHP `[WPSG]` prefixes.** Alert email subjects (`[WPSG] Fatal error detected` / `[WPSG] REST error spike detected`, asserted in `Mullion_Alerts_Test`), `error_log('[WPSG] …')` in the logger and font-library, adapter-schema exception messages, `_doing_it_wrong(…, 'Unknown WPSG permission action: …')`, cron schedule display `Every 5 minutes (WPSG alerts)`. P74-J only swept JS console prefixes.
4. **P74-B, now stale.** `readme.txt` install step still says Navigate to **WP Super Gallery**. P74-E set CPT `menu_name` to **Mullion**.
5. **Tooling path leftovers P74-A scoped out of itself.** `.claude/skills/php-testing/SKILL.md` and `see-wp/SKILL.md` still `wp-plugin/wp-super-gallery` / `WPSG_*_Test`. `CONTRIBUTING.md` still `cd wp-plugin/wp-super-gallery`. These break the php-testing skill; they are not `docs/`.

**The migrator / uninstall / P52 leftover path is not in that list by accident.** Those files still contain `wpsg_*` because they are the FROM side of a rewrite map (Key Decision E): find `wpsg_campaign` rows, rename them to `mullion_campaign`. That is not a public alias like the shortcode P74-D dropped. Mechanically renaming those strings would make the migrator look for rows that already have the new names and leave any leftover old rows untouched.

Keeping that FROM-map in shipped PHP forever still fails the phase success criterion ("every `wpsg` identifier in shipped code reads as `mullion`"). Pre-launch, there are no public installs. Local wp-env data is reseedable. The way to stop shipping the old prefix is to **delete the machinery**, not to rename its keys. Same call as P74-D. Recorded as Key Decision H.

### Fix

- **Drop, do not rename:**
  - `includes/class-mullion-rebrand-migration.php` and `tests/Mullion_Rebrand_Migration_Test.php`; unhook `Mullion_Rebrand_Migration::maybe_run` from `plugins_loaded`.
  - Uninstall old-name dual-lookups (old post types, taxonomies, roles, `LIKE 'wpsg\_%'`, old tables/indexes/upload dirs). Uninstall only the current `mullion_*` surface.
  - `mullion_maybe_migrate_roles()` and its `wpsg_admin` lookups. Keep `mullion_ensure_editor_role()` (current role, not a leftover). Rewrite `Mullion_P52A2_Role_Migration_Test` to cover current `mullion_editor` behaviour only — do not seed `wpsg_admin`.
- **Rename the missed live identifiers:**
  - `release.yml` bump step: `MULLION_VERSION` (comments too).
  - `wpsgSettingsAuthTest` → `mullionSettingsAuthTest`; `dataset.wpsgBound` → `dataset.mullionBound`.
  - PHP `[WPSG]` / `WPSG` live strings and remaining PHP comments that still say WPSG (`class-mullion-alerts.php`, logger, font-library, adapter-field-schema, permissions, image-optimizer, rest-base, asset-library, campaign-status, CLI, schema JSON description, test comments). Alert-test assertion follows the new subject.
  - `readme.txt` install step: **Mullion**. Leave `Contributors: wpsupergallery` and changelog history lines.
  - phpcs ruleset display name; GitHub Actions workflow comment titles (already in `release.yml`).
  - `.claude/skills/php-testing/SKILL.md`, `see-wp/SKILL.md`, `CONTRIBUTING.md` path / test-class literals.

**Keep:** `Mullion_License_Test`'s negative `'slug' => 'wp-super-gallery'` assertion (P74-K CI gate). Freemius `is_premium` / `has_premium_version` / `is_org_compliant` / `premium_slug` / `menu['first-path']` (P75-A/M2). `languages/` GitHub-URI msgids (P74-C `make-pot` backlog). `docs/**` prose (P74-M). `.wordpress-org/*`.

### Acceptance criteria

- `grep -n "wpsg\|WPSG\|wp-super-gallery\|WP Super Gallery"` across `wp-plugin/mullion-gallery/` (excl. `vendor/`, `languages/`, the P74-K license-test negative assertion, and `readme.txt` changelog history) returns zero results.
- `release.yml` bumps `MULLION_VERSION`. Settings auth-test localize object is `mullionSettingsAuthTest`.
- No `Mullion_Rebrand_Migration` class, no `mullion_maybe_migrate_roles`, no uninstall `wpsg_*` lookups.
- `.claude/skills/php-testing/SKILL.md` and `CONTRIBUTING.md` `cd` into `wp-plugin/mullion-gallery`.
- Full PHPUnit stays green (test count drops by the deleted migrator / P52-legacy cases). Full Vitest unchanged.

### Validation

- Full PHPUnit via `/php-testing`: **1,303 tests, 13,676 assertions, 2 skipped, 0 failures.** Was 1,314 / 13,741 before this track (dropped `Mullion_Rebrand_Migration_Test` plus two P52 leftover-role cases; two pre-existing skips unchanged).
- `php -l` on edited PHP: clean.
- Acceptance grep: remaining `wpsg`/`WPSG`/`wp-super-gallery` in `wp-plugin/mullion-gallery/` is the P74-K license-test negative assertion, `languages/` GitHub-URI msgids / stale `#:` comments, and `readme.txt` changelog history. Workflows, skills, and CONTRIBUTING are clean.

### Implementation Notes (2026-08-25)

- **Dropped, did not rename.** Deleted `includes/class-mullion-rebrand-migration.php` and `tests/Mullion_Rebrand_Migration_Test.php`; unhooked `plugins_loaded` require. Uninstall now only deletes the current `mullion_*` surface (posts, taxonomies, options, transients, tables, indexes, roles, upload dirs). Added `mullion_rebrand_migration_version` to the uninstall option list so leftover E/F flags from already-migrated local DBs still get cleaned. Removed `mullion_maybe_migrate_roles()` and its `init` hook. Kept `mullion_ensure_editor_role()`.
- **P52 tests rewritten, not gutted.** Dropped the two tests that seeded `wpsg_admin`. Kept editor-cap tests and `/users` accept-`mullion_editor`. Replaced reject-legacy-role with `test_create_user_rejects_unknown_role` (`not_a_role`) so the enum still has a negative case without shipping the old slug. Roles-list test now only asserts `mullion_editor` is offered.
- **Live identifier misses patched.** `release.yml` bump/`grep` target `MULLION_VERSION`; workflow comment titles Mullion. `wpsgSettingsAuthTest` / `dataset.wpsgBound` → `mullionSettingsAuthTest` / `dataset.mullionBound`. PHP `[WPSG]` email subjects, Sentry messages, `error_log` prefixes, adapter-schema exceptions, `_doing_it_wrong` text, cron display name, and remaining PHP comments. `readme.txt` install step **Mullion**. phpcs ruleset display name. CONTRIBUTING paths (including the in-container `wp-content/plugins/…` cd). php-testing / see-wp skills updated locally; `.claude/` is gitignored so those edits are not in the commit.
- **Kept, as specified.** License-test negative `'slug' => 'wp-super-gallery'`. Freemius structural flags. `languages/` GitHub-URI msgids and stale `#: class-wpsg-*.php` source comments (P74-C `make-pot` backlog). `readme.txt` changelog history (`wp wpsg`, `@wpsg`). `Contributors: wpsupergallery`. `docs/**` (P74-M). `.wordpress-org/*`.
- **Tradeoff.** Local wp-env databases that still hold `wpsg_*` post types / options are **not** rewritten — reseed or wipe.
...
Roughly 149 files under `docs/` mention "WP Super Gallery" or `wpsg` in prose. The root [README.md](../README.md) still carries the project's original working name, `# campaignViewerWP`, predating even the WP Super Gallery name. `.wordpress-org/README.md` and `docs/design/DESIGN_BRIEF.md` both describe the product under its old name throughout.
...
- `.wordpress-org/README.md`, `docs/design/DESIGN_BRIEF.md` — full rename pass, plus append the color-system explanation and the ink-safe rejection to the design brief (see the companion update landing alongside this phase doc).
...
- **`docs/design/DESIGN_BRIEF.md` not staged.** Already named Mullion in HEAD; the working-tree dirty diff is P74-N/designer-collab status that links untracked `COLOR-SPEC.md` / response files. Rename pass for listing copy is `.wordpress-org/README.md` + `design-brief.html`.
...
- **Not touched:** `docs/archive/**`; license-test negative `'slug' => 'wp-super-gallery'`; `languages/` GitHub-URI msgids / stale `#:` comments (P74-C); `readme.txt` changelog history; `Contributors: wpsupergallery`; untracked `docs/design/COLOR-SPEC.md` and designer-response files.
...
The current default theme (`default-dark.json`, id `default-dark`) carries the old brand's navy-blue palette (`#0f172a` / `#3b82f6`). The designer's original submission ("ink-safe," `#0f857c`) failed this repo's own WCAG AA gate — see Key Decision G. Since then, two further review rounds with the designer produced a complete, self-verified palette (`docs/design/COLOR-SPEC.md`), superseding everything below the original four-role submission.
...
- **`primaryShade` is absent, not TBD-with-a-placeholder, per final designer review (round 6) — and this sub-item is no longer P74-N's to close (Decision I).** Matching the theme JSON they now ship: no `primaryShade` value at all, replaced with a `_primaryShade` note carrying the criterion (first array index from the dark end clearing 4.5:1 against the lightest surface and under white text — expressed as a plain array index, not a Tailwind-style rung name; see the note below on why), the measured current-HSL values, and an explicit "owned by P75-F." Nothing sits in the file that could be copied out and used by mistake. Under the current HSL generator the criterion resolves at **index 7** (`#0f7971`, 4.79:1 on text, 5.26:1 under white) — verified directly against `generateColorScale()`, not modeled. The OKLCH-side answer is not yet derivable at all; it depends on lightness stops P75-F hasn't chosen. **P75-F sets the live value** when it swaps the ramp generator, in the same commit that re-derives the other 16 themes' indices.
...
1. **Full-repo identifier sweep.** After P74-Q: `grep -ri "wpsg\|wp-super-gallery" --include=*.php --include=*.ts --include=*.tsx --include=*.js --include=*.yml` across `wp-plugin/mullion-gallery/` (excl. `vendor/`, `languages/`, the P74-K license-test negative assertion, `readme.txt` changelog history), `src/`, `packages/*/src/`, `e2e/`, `public/sw.js`, `.github/workflows/` returns zero results.
...
4. **PHPUnit**, via the `php-testing` skill (wp-env) — full suite. The P74-E/F migration test is deleted by Q (Decision H); do not re-seed old-prefix fixture data.
5. **Vitest**, full suite, plus `auditThemeContrast` run directly against the finalized theme JSON.
6. **wp-env manual QA.** Boot the renamed plugin, confirm the admin menu, the `[mullion-gallery]` shortcode, and the front-end gallery. Pre-rename database snapshots are **not** migrated after Q — wipe or reseed.
7. **Playwright visual regression**, diffed against pre-rename baselines — expect zero diffs from the identifier-only tracks (P74-A through P74-M/Q), and intentional diffs only from P74-N/O, reviewed and re-baselined.
