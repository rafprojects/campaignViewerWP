...
## PR Review Resolution (2026-06-13)

A self-review of the branch (`docs/PHASE50_PR_REVIEW.md`) raised 8 findings. All addressed in a single follow-up pass; `tsc -b` clean, frontend `vitest run` 2309/2309, PHP PHPUnit 951 OK / 2 skipped — all unchanged from the review baseline.

- **#1 (Medium) — SW SWR intercepted the admin campaign list.** `META_ENDPOINT_RE` tests `url.pathname` only, so `/campaigns?include_archived=…` (admin list) matched and went stale-while-revalidate — a moved/created/archived campaign could read stale until a second fetch, undermining the move feature this branch ships. *Fix* (`public/sw.js`): the `fetch` handler now bypasses the meta cache (→ network-first) whenever the request carries `X-WP-Nonce` or `Authorization`, so every authenticated/admin/mutation flow stays network-first per Key Decision D; only anonymous public reads keep SWR. *Rationale:* gating on the auth header (rather than reshaping the regex) is robust to future query-param shapes and exactly matches the "public read" boundary.
- **#2 (Medium) — move transaction relied on an implicit InnoDB engine.** `move_campaign_to_space()` wraps five writes in `START TRANSACTION`/`ROLLBACK`, a silent no-op on a MyISAM-default server. *Fix* (`class-wpsg-db.php`): the four campaign-scoped `CREATE TABLE`s now pin `ENGINE=InnoDB` (new installs); `DB_VERSION` 14→15 with a guarded, idempotent `maybe_convert_campaign_tables_to_innodb_v15()` that reads `information_schema.TABLES` and only `ALTER … ENGINE=InnoDB` when a table isn't already InnoDB (existing installs); the `move_campaign_to_space` docblock now states the InnoDB requirement. *Rationale:* `dbDelta` won't alter engine on existing tables, so the CREATE pin alone wouldn't reach already-installed sites — hence the option-gated conversion migration.
- **#3 (Low) — `Service-Worker-Allowed: /` was documented but never sent.** *Fix* (`class-wpsg-embed.php`): `maybe_serve_service_worker()` now emits the header, making the docblock and the `main.tsx` comment accurate and future-proofing explicit broader-scope registration (preferred over deleting the comments).
- **#4 (Low) — SW eviction mislabeled.** Doc-only: corrected `public/sw.js` comments to describe plain FIFO first-insertion eviction with a flat 50-entry global cap (not "LRU approximation" / "5 MB per space"). No behavior change.
- **#5 (Low) — `SHOW TABLES LIKE` didn't escape the prefix `_`.** *Fix* (`class-wpsg-db.php`): `maybe_rename_overlays_to_assets_v14()` now uses `$wpdb->prepare('SHOW TABLES LIKE %s', $wpdb->esc_like($table))` for both checks; the `=== $old`/`=== $new` equality guards already made false positives harmless, so this is a precision nit.
- **#6 (Low) — shared-utils tests stranded in the app tree.** *Fix:* moved the four test files into `packages/shared-utils/src/` (co-located with sources, imports switched to relative `./module`); broadened the root vitest `include` in `vite.config.ts` to also match `packages/**/*.test.{ts,tsx}`. The package is now self-contained; suite count unchanged (the 57 tests run from the new location).
- **#7 (Low) — move audit recorded `from_space_id: 0` for pre-backfill campaigns.** *Fix* (`class-wpsg-campaign-controller.php`): `move_campaign()` now falls back to `wpsg_default_space_id` when the source meta is absent, mirroring `require_campaign_space_move()` so the no-op check and the audit record both reflect the real source space.
- **#8 (Pre-existing) — `console.log` in production at `main.tsx:353-354`.** Already resolved: both logs sit inside an `if (import.meta.env.DEV)` block on the current branch; verified no ungated `console.*` remains in the file. No change.
