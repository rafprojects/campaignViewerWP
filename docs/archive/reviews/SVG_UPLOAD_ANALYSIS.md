...
*Decision: **Option 2 (sanitize and allow)** selected on March 5, 2026 with enhancements: dual-layer sanitization (`enshrined/svg-sanitize` server-side + DOMPurify client-side), custom CSS sanitizer for `<style>` blocks, URI allowlisting, `@font-face` with embedded data: fonts only. Implemented as Track P20-L in [PHASE20_REPORT.md](../phases/PHASE20_REPORT.md).*
