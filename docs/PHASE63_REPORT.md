...
Terminology note settled during review: export is an **editor/admin-only** capability. Every export create-gate and the `export_jobs.*` read/delete/download gates require at least `manage_wpsg` (a system admin has it too). A content **reader / TIER_VIEWER** — no `manage_wpsg` — is rejected at the `permission_callback` and never reaches this logic. "Requesting editor" throughout this track means the manage_wpsg/admin actor calling the endpoint, **not** a content viewer nor the per-space `viewer` grant-level. Only `manage_options` (System Admin) is owner in every space; per **P53-A** a `manage_wpsg` editor has access to a space **only via an explicit grant — in *both* `open` and `delegated` isolation modes** (open mode does *not* confer implicit editor access; `isolation_mode` governs asset visibility, not this gate — see `WPSG_P47_Spaces_Isolation_Test::test_open_space_denies_manage_wpsg_without_grant`). So the space gate bites any editor lacking a grant to a contributing space — most visibly when a grant is **revoked** after the job was created.
...
Automated coverage (the 7 `WPSG_P63*` test files + the extended rate-limiter/coverage suites) is the primary gate. The steps below are the manual/end-to-end confirmations that automated tests can't fully capture, plus the rationale where manual QA is not applicable. All manual steps assume the local wp-env dev site (`npx @wordpress/env`), and for the header/asset checks a **built** plugin (`npm run build:wp`) so `assets/` (and `assets/.htaccess`) exist.

> **Detailed runbook:** the table below is the summary. For step-by-step instructions — exact commands, persona setup, authentication, expected results, the reasoning that makes each result meaningful, and the pitfalls that silently invalidate a test — see **[PHASE63_MANUAL_QA_RUNBOOK.md](PHASE63_MANUAL_QA_RUNBOOK.md)**.
...
| **P63-I** | Grant a `manage_wpsg` editor spaces A **and** B (grants are what gate access — the space's isolation mode is irrelevant) and, as that editor, run a **batch** binary export spanning a space-A and a space-B campaign (note the job ID). Now **revoke that editor's grant to space B** and, as the same editor, `GET`/`download` the job → `403` — they still own it but no longer hold every contributing space (this isolates the new space gate from the P63-E-2 ownership gate). Re-grant space B → `200`/download works. Single-campaign variant: an editor who exported a space-A campaign can read/download it; revoke their space-A grant → `403`. | A **System Admin** reads/downloads any job (owner everywhere); an editor who retains all contributing-space grants keeps full access; `audit` / `media-library` and spaceless/legacy jobs (no stamped space) are unaffected; P63-E tier + P63-E-2 ownership behavior unchanged (a non-creator peer is still denied by ownership before the space gate is reached). |
