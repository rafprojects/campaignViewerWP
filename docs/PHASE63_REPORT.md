...
| **P63-I** | In **delegated-space** mode, grant an editor spaces A **and** B and, as that editor, run a **batch** binary export spanning a space-A and a space-B campaign (note the job ID). Now **revoke that editor's grant to space B** and, as the same editor, `GET`/`download` the job → `403` — they still own it but no longer hold every contributing space (this isolates the new space gate from the P63-E-2 ownership gate). Re-grant space B → `200`/download works. Single-campaign variant: an editor who exported a space-A campaign can read/download it; revoke their space-A grant → `403`. | A **System Admin** reads/downloads any job (owner everywhere); an editor who retains all contributing-space grants keeps full access; `audit` / `media-library` and spaceless/legacy jobs (no stamped space) are unaffected; P63-E tier + P63-E-2 ownership behavior unchanged (a non-creator peer is still denied by ownership before the space gate is reached). |
