...
> **Side note / potential follow-up:** while wiring the PHP side I noticed the *existing* spotlight and scroll-snap adapter flat keys (`spotlight_strip_position`, `spotlight_thumbnail_size`, `scroll_snap_alignment`, …) are **not** present in the registry `$defaults`, which the nested sanitizer requires — so those may be silently dropped on save through that path. Out of scope for P51-E (the new key is correctly defaulted), but worth auditing separately.
