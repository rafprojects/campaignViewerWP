...
class Mullion_CLI {
    ...
    public function campaign_export( array $args, array $assoc_args ): void {
        ...
        if ( $format === 'binary' ) {
            ...
            $job_id = Mullion_Export_Engine::create_job( 'campaign', $manifest, (array) $media );
    ...
    private function campaign_import_binary( string $file ): void {
        ...
        $result = Mullion_Campaign_IO::import_entry( $entry, $zip, [ 'via' => 'cli', 'format' => 'binary' ] );
        $zip->close();
    ...
    /**
     * Record a campaign audit-log entry via the shared Mullion_REST method.
     */
    private function add_audit_entry( int $campaign_id, string $event, array $data = [] ): void {
        Mullion_REST::add_audit_entry( $campaign_id, $event, $data );
    }
