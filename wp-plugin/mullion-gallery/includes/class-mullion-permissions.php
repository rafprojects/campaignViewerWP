...
/**
 * Mullion_Permissions — centralized authorization map (P52-A).
 *
 ...
 * {@see Mullion_Permissions::gate()}, so this MAP is the complete, auditable
 ...
 * *strategy*: the name of a permission primitive declared on Mullion_REST_Base
 ...
 * a one-line MAP edit asserted by Mullion_P52A_Permission_Matrix_Test.
 *
 * @since P52-A1
 */
final class Mullion_Permissions {
    ...
    public static function check(string $action, $request = null) {
        ...
        // All strategies are public static primitives on Mullion_REST_Base. Those
        // that ignore the request (require_admin, require_authenticated) accept
        // the extra argument harmlessly; the scoped ones read it.
        return call_user_func([Mullion_REST_Base::class, $strategy], $request);
    }

    /**
     * Build a permission_callback bound to a specific action. Used at route
     * registration: `'permission_callback' => Mullion_Permissions::gate('foo.bar')`.
     *
