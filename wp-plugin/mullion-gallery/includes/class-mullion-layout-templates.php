...
class Mullion_Layout_Templates {
     ...
     * Mullion_Permissions: a permission failure 403s the request, but an
    ...
    private static function enforce_license_gates( array $data, ?array $existing = null ): array {
        if ( ! Mullion_License::can_use_feature( Mullion_License::FEATURE_LAYOUT_TEXT_LAYERS ) ) {
            $data['texts'] = $existing === null ? [] : ( $existing['texts'] ?? [] );
        }
        if ( ! Mullion_License::can_use_feature( Mullion_License::FEATURE_LAYOUT_BREAKPOINT_OVERRIDES ) ) {
