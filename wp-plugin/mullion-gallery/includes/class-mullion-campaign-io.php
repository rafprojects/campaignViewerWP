<?php
/**
 * Mullion_Campaign_IO — single source of truth for campaign export/import.
 *
 * P65-A (C-1): the campaign import/export pipeline previously existed in four
 * drifting copies — REST JSON (`Mullion_Export_Controller::import_campaign` /
 * `export_campaign`), REST ZIP (`import_single_campaign_from_zip` /
 * `export_campaign_binary`), CLI JSON and CLI ZIP (`Mullion_CLI::campaign_import*`
...
class Mullion_Campaign_IO {
    ...
    public static function build_entry(int $post_id, bool $binary = false): array {
        ...
        $campaign = Mullion_REST_Base::format_campaign($post);
        ...
        $media_references = array_values(array_map(function ($item) use ($binary) {
            ...
            if ($binary) {
                $ref['filename'] = Mullion_Export_Engine::get_media_filename($item);
            }
            return $ref;
        }, $media));
    ...
    public static function import_entry(array $entry, ?ZipArchive $zip = null, array $opts = []) {
        ...
        Mullion_REST_Base::add_audit_entry($post_id, 'campaign.imported', [
            'source_title' => $title,
            'format'       => $format,
     ...
     * it to the campaign. A-4 fix: always `Mullion_Layout_Templates::create()`,
    ...
    private static function apply_layout_template(int $post_id, array $src, $layout_template): void {
        ...
        $created = Mullion_Layout_Templates::create($layout_template);
    ...
    private static function sideload_media_items(int $post_id, array $media_references, ZipArchive $zip): array {
        ...
        foreach ($media_references as $ref) {
            ...
            $md5         = md5_file($tmp) ?: '';
            $existing_id = $md5 ? Mullion_REST_Base::find_attachment_by_md5($md5) : 0;
            if ($existing_id > 0) {
