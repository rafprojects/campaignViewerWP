...
class Mullion_Monitoring {
    ...
    public static function get_health_data() {
        ...
        $cache_stats = [];
        if (class_exists('Mullion_Thumbnail_Cache')) {
            $cache_stats = Mullion_Thumbnail_Cache::get_stats();
        }
        ...
        $recent_logs = class_exists('Mullion_Logger') ? Mullion_Logger::get_recent_logs(50) : [];

        // P39-IN1: Webhook delivery summary.
        $webhook_deliveries = class_exists('Mullion_Webhooks') ? Mullion_Webhooks::get_delivery_log() : [];
        $webhook_endpoint_count = class_exists('Mullion_Webhooks') ? count(Mullion_Webhooks::get_endpoints()) : 0;
        ...
        return [
            ...
            'pluginVersion'                => defined('MULLION_VERSION') ? MULLION_VERSION : 'unknown',
            'timestamp'                    => time(),
