...
class Mullion_Webhooks {
    ...
    public static function dispatch(string $event, array $payload) {
        ...
        // P67-H: dispatch delivery attempt #1 through WP-Cron (like retries 2-3)
        // instead of blocking the originating request. A slow or unreachable
        // endpoint used to add up to MAX_ENDPOINTS × 10s to every campaign/media
    ...
    public static function deliver(int $idx, array $endpoint, string $event, array $full_payload, int $attempt) {
        ...
        $response = wp_safe_remote_post($url, [
            ...
            'headers'   => [
                ...
                'X-MULLION-Signature' => $signature,
                'X-MULLION-Event'     => $event,
                'X-MULLION-Delivery'  => $delivery_id,
                'X-MULLION-Attempt'   => (string) $attempt,
            ],
            'body'      => $body,
            'sslverify' => true,
        ]);
