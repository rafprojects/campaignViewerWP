...
/**
 * Mullion_Export_Engine — reusable background export job manager.
 *
...
class Mullion_Export_Engine {
    ...
    public static function create_job(
        ...
        string $required_tier = Mullion_Permissions::TIER_EDITOR,
        array $space_ids = []
    ): string {
        $id = bin2hex(random_bytes(16));

        $allowed_tiers = [
            Mullion_Permissions::TIER_SYSTEM_ADMIN,
            Mullion_Permissions::TIER_EDITOR,
            Mullion_Permissions::TIER_VIEWER,
        ];
        if (!in_array($required_tier, $allowed_tiers, true)) {
            $required_tier = Mullion_Permissions::TIER_EDITOR;
        }
     ...
     * of entry size. Shared by campaign ZIP import (Mullion_Campaign_IO) and the
