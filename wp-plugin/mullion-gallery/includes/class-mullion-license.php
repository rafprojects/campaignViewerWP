 ...
 * the pro payload (see Mullion_Layout_Templates). Do NOT fold entitlement into
 * Mullion_Permissions::MAP.
 *
 * ── Credential-ready (mirrors Mullion_Sentry / mullion_sentry_dsn) ────────────────
 * With no Freemius credentials configured, mullion_fs() (mullion-gallery.php)
 ...
 * `add_filter('mullion_license_is_pro', '__return_true')` — with zero real
 ...
 * `mullion_freemius_config` filter) and are never committed.
 *
 * @since P62-A
 */
final class Mullion_License {
     ...
     * Shared with mullion-gallery.php's mullion_fs() bootstrap via the same filter.
     *
    ...
    public static function is_sdk_active(): bool {
        return function_exists('mullion_fs') && mullion_fs() !== null;
    }
    ...
    public static function can_use_premium_code(): bool {
        if (self::is_sdk_active()) {
            return (bool) mullion_fs()->can_use_premium_code();
        }
        // Stub path: default false (safe/free) until wired to a real license.
        return (bool) apply_filters('mullion_license_is_pro', false);
    }
    ...
    public static function can_use_feature(string $feature): bool {
        return (bool) apply_filters('mullion_license_feature_enabled', self::can_use_premium_code(), $feature);
    }
    ...
    public static function get_tier(): ?string {
        if (self::is_sdk_active() && method_exists(mullion_fs(), 'get_plan')) {
            $plan = mullion_fs()->get_plan();
            if ($plan && !empty($plan->name)) {
                return (string) $plan->name;
            }
            return null;
        }
        $tier = apply_filters('mullion_license_tier', null);
     ...
     * the `mullion_license_upgrade_url` filter is never set (P62-K); otherwise it falls
    ...
    public static function get_upgrade_url(): string {
        ...
        if (self::is_sdk_active() && method_exists(mullion_fs(), 'get_upgrade_url')) {
            $sdk_url = mullion_fs()->get_upgrade_url();
            if (is_string($sdk_url) && $sdk_url !== '') {
                $default = $sdk_url;
            }
        }
        return (string) apply_filters('mullion_license_upgrade_url', $default);
    }
}
