...
class Mullion_Export_Controller extends Mullion_REST_Base {
    ...
    public static function register_routes(): void {
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/batch/export/binary', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaigns.batch.export_binary'),
                'args'                => [
                    'ids' => [
                        'required'  => true,
                        'type'      => 'array',
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/import', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaigns.import'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/export-jobs/(?P<job_id>[a-f0-9]{32})', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('export_jobs.read'),
            ],
            [
                'methods'             => 'DELETE',
        ...
        register_rest_route('wp-super-gallery/v1', '/export-jobs/(?P<job_id>[a-f0-9]{32})/download', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('export_jobs.download'),
            ],
        ]);
    }
    ...
    public static function export_campaign($request) {
        ...
        // fixes A-4 (layout template embedded via Mullion_Layout_Templates::get,
        // not the old get_post(intval($uuid)) that always yielded null).
        $entry    = Mullion_Campaign_IO::build_entry($post_id, false);
    ...
    public static function import_campaign($request) {
        ...
        // the layout template is created via Mullion_Layout_Templates::create()
        ...
        $result = Mullion_Campaign_IO::import_entry($entry, null, ['via' => 'rest', 'format' => 'json']);
    ...
    public static function export_campaign_binary($request) {
        ...
        $entry    = Mullion_Campaign_IO::build_entry($post_id, true);
        $campaign = $entry['campaign'];
        ...
        $job_id   = Mullion_Export_Engine::create_job(
            'campaign',
            $manifest,
            (array) $media,
    ...
    public static function batch_export_binary($request) {
        ...
        foreach ($ids as $post_id) {
            ...
            $campaigns_data[] = Mullion_Campaign_IO::build_entry($post_id, true);
            ...
            foreach ($media as $item) {
                ...
                if ($url && !isset($seen_urls[$url])) {
                    ...
                    $canonical_filenames[$url] = Mullion_Export_Engine::get_media_filename($item);
                }
            }
        }
    ...
    private static function authorize_job_access(array $job) {
        $required = $job['required_tier'] ?? Mullion_Permissions::TIER_EDITOR;
        if (!Mullion_Permissions::actor_has_tier($required)) {
    ...
    public static function import_campaign_binary($request) {
        ...
        if ($version === 2) {
            ...
            $result = Mullion_Campaign_IO::import_entry($entry, $zip, ['via' => 'rest', 'format' => 'binary']);
            $zip->close();
            if (is_wp_error($result)) {
                return $result;
            }
