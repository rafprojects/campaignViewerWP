...
class Mullion_Media_Controller extends Mullion_REST_Base {
    ...
    public static function register_routes(): void {
        ...
        register_rest_route('wp-super-gallery/v1', '/media/(?P<mediaId>[a-zA-Z0-9_.-]+)/usage', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('media.usage.read'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)/media', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.media.list'),
                'args'                => [
                    'sort' => [
                        'type'    => 'string',
            ...
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.media.create'),
                'args'                => [
                    'type'   => [
                        'required' => true,
                        'type'     => 'string',
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)/media/batch', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.media.create_batch'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)/media/reorder', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.media.reorder'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)/media/rescan', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.media.rescan'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)/media/(?P<mediaId>[a-zA-Z0-9_-]+(?:\.[a-zA-Z0-9_-]+)*)', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.media.update'),
            ],
            [
                'methods' => 'DELETE',
                // P33-C: editor and owner can delete media items.
                'callback' => [self::class, 'delete_media'],
                'permission_callback' => Mullion_Permissions::gate('campaign.media.delete'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/media/rescan-all', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('media.rescan_all'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/media/library', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('media.library.list'),
                'args'                => [
                    'sort' => [
                        'type'    => 'string',
                        'enum'    => ['order_asc', 'order_desc', 'title_asc', 'title_desc', 'created_asc', 'created_desc', 'size_asc', 'size_desc'],
        ...
        register_rest_route('wp-super-gallery/v1', '/media/upload', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('media.upload'),
                'args'                => [
                    'force' => [
                        'type'    => 'boolean',
                        'default' => false,
        ...
        register_rest_route('wp-super-gallery/v1', '/tags/media', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('media_tags.list'),
            ],
            [
                'methods' => 'POST',
                'callback' => [self::class, 'create_media_tag'],
                'permission_callback' => Mullion_Permissions::gate('media_tags.create'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/tags/media/(?P<id>\d+)', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('media_tags.delete'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/admin/media/export/binary', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('media.export_binary'),
                'args'                => [
        ...
        register_rest_route('wp-super-gallery/v1', '/media/import/binary', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('media.import_binary'),
            ],
        ]);
    }
    ...
    private static function get_max_batch_upload_size() {
        $configured_limit = intval(Mullion_Settings::get_setting('max_batch_upload_size', 20));
    ...
    private static function find_near_duplicates_by_phash(string $phash, int $threshold): array {
        ...
        foreach ($rows as $row) {
            $d = Mullion_PHash::hamming_distance($phash, (string) $row['meta_value']);
            if ($d === 0) {
    ...
    private static function find_attachment_origin_meta(int $id): array {
        ...
        $campaigns = class_exists('Mullion_DB') ? Mullion_DB::get_campaigns_for_attachment_id($id) : [];
    ...
    public static function rescan_all_media_types() {
        $campaigns = get_posts([
            'post_type' => Mullion_CPT::POST_TYPE,
            'posts_per_page' => -1,
    ...
    private static function normalize_external_media($url) {
        ...
        if ($host === 'rumble.com' || $host === 'www.rumble.com') {
            ...
            // P67-E: shared video-ID token (see Mullion_Provider_Registry::RUMBLE_VIDEO_ID_TOKEN).
            if (!preg_match('/^' . Mullion_Provider_Registry::RUMBLE_VIDEO_ID_TOKEN . '$/i', $video_id)) {
    ...
    public static function export_media_library_binary($request) {
        ...
        // uniqid string that intval()s to 0 for every item (as Mullion_CLI::media_orphans()
        ...
        foreach ((array) $query->posts as $post) {
            ...
            $manifest_items[] = [
                'id'       => (string) $att_id,
                'filename' => Mullion_Export_Engine::get_media_filename($engine_item),
                'url'      => $url,
                'title'    => $post->post_title,
                'mimeType' => $mime,
            ];
        ...
        $job_id = Mullion_Export_Engine::create_job('media_library', $manifest, $media_items, required_tier: Mullion_Permissions::TIER_SYSTEM_ADMIN);
    ...
    public static function import_media_library_binary($request) {
        ...
        foreach ((array) ($manifest['items'] ?? []) as $item) {
            ...
            // fails.
            if (!Mullion_Export_Engine::stream_zip_entry_to_file($zip, 'media/' . $filename, $tmp)) {
                wp_delete_file($tmp);
                $skipped[] = $filename;
                continue;
            }
