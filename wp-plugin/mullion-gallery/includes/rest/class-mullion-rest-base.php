 ...
 */
abstract class Mullion_REST_Base {
    ...
    public static function rate_limit_authenticated($request) {
        if (!Mullion_Permissions::actor_has_tier(Mullion_Permissions::TIER_SYSTEM_ADMIN)) {
            return false;
        }
        ...
        $limit = $global_limit;
        if ($space_id > 0 && class_exists('Mullion_Settings')) {
            $space_settings = Mullion_Settings::get_effective_settings($space_id);
    ...
    public static function require_admin() {
        if (!Mullion_Permissions::actor_has_tier(Mullion_Permissions::TIER_EDITOR)) {
            return false;
        }
    ...
    protected static function validate_access_level($level): string {
        // P64-A: Mullion_Grants is the canonical home for this rule; delegate.
        return Mullion_Grants::validate_access_level($level);
    }
    ...
    protected static function get_effective_space_level(int $user_id, int $space_id): string {
        ...
        if (is_array($grants)) {
            foreach ($grants as $grant) {
                ...
                if (Mullion_Grants::is_expired($grant)) {
    ...
    public static function require_space_admin(WP_REST_Request $request): bool {
        ...
        $user_id = get_current_user_id();
        if ($user_id <= 0 || !Mullion_Permissions::actor_has_tier(Mullion_Permissions::TIER_EDITOR)) {
            return false;
        }
        $space_id = intval($request->get_param('id'));
        if ($space_id <= 0) {
    ...
    private static function verify_admin_auth() {
        ...
        // Allow nonce bypass ONLY when the explicit MULLION_ALLOW_NONCE_BYPASS
        ...
        if ( defined( 'MULLION_ALLOW_NONCE_BYPASS' ) && MULLION_ALLOW_NONCE_BYPASS ) {
            ...
            if ( ! $is_test_env ) {
                ...
                Mullion_Logger::warning( 'security', 'MULLION_ALLOW_NONCE_BYPASS is enabled outside a recognized test environment', [ 'constant' => 'MULLION_ALLOW_NONCE_BYPASS' ] );
            }
            if ( $is_test_env ) {
                return true;
            }
    ...
    public static function require_authenticated() {
        return Mullion_Permissions::actor_has_tier(Mullion_Permissions::TIER_VIEWER);
    }
    ...
    protected static function guard_admin_only_settings(array $snake_input) {
        ...
        $admin_only = array_flip(Mullion_Settings_Registry::get_admin_only_fields());
        $requested  = array_intersect_key($snake_input, $admin_only);
        if (empty($requested)) {
            return null;
        }

        $sanitized = Mullion_Settings::sanitize_settings($snake_input);
        $current   = Mullion_Settings::get_settings();
        $blocked   = [];
    ...
    protected static function write_global_settings(
        ...
        update_option(Mullion_Settings::OPTION_NAME, array_merge($current, $to_merge));
    ...
    // P65-A: public so the standalone Mullion_Campaign_IO service (not a subclass)
    ...
    // ── Campaign formatting ───────────────────────────────────────────────────
    // Shared by Mullion_Campaign_Controller, Mullion_Export_Controller, and any future
    ...
    public static function promote_campaign_gallery_overrides($gallery_overrides) {
        $sanitized = Mullion_Settings_Sanitizer::sanitize_gallery_overrides($gallery_overrides);
    ...
    // ── Media deduplication ───────────────────────────────────────────────────
    // Shared by Mullion_Export_Controller (binary import) and Mullion_Media_Controller
    // (single-file upload).
    ...
    // ── Taxonomy term helpers (used by Mullion_Content_Controller and Mullion_Media_Controller) ──
