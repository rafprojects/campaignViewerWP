...
class Mullion_Campaign_Controller extends Mullion_REST_Base {
    ...
    public static function register_routes(): void {
        register_rest_route('wp-super-gallery/v1', '/campaigns', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaigns.list'),
            ],
            [
                'methods'             => 'POST',
                'callback'            => [self::class, 'create_campaign'],
                'permission_callback' => Mullion_Permissions::gate('campaigns.create'),
                'args'                => [
                    'title'       => [
                        'required'          => true,
                        'type'              => 'string',
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.read'),
            ],
            [
                'methods'             => 'PUT',
                // P33-C: editor and owner can update campaign metadata.
                'callback'            => [self::class, 'update_campaign'],
                'permission_callback' => Mullion_Permissions::gate('campaign.update'),
                'args'                => [
                    'title'      => [
                        'type'              => 'string',
                        'sanitize_callback' => 'sanitize_text_field',
            ...
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.delete'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)/archive', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.archive'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)/restore', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.restore'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)/duplicate', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.duplicate'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)/move', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.move'),
                'args'                => [
                    'target_space_id' => [
                        'required' => true,
                        'type'     => 'integer',
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/batch', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaigns.batch'),
                'args'                => [
                    'action' => [
                        'required' => true,
                        'type'     => 'string',
        ...
        register_rest_route('wp-super-gallery/v1', '/campaigns/(?P<id>\d+)/audit', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('campaign.audit.read'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/admin/audit-log', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('system.audit_log.read'),
            ],
        ]);
        ...
        register_rest_route('wp-super-gallery/v1', '/admin/audit-log/export/binary', [
            [
                ...
                'permission_callback' => Mullion_Permissions::gate('system.audit_log.export_binary'),
                'args'                => [
    ...
    public static function delete_campaign($request) {
        ...
        if ($purge_analytics) {
            global $wpdb;
            $wpdb->delete(
                Mullion_DB::get_analytics_table(),
                ['campaign_id' => $post_id],
                ['%d']
            );
        }
    ...
    public static function duplicate_campaign($request) {
        ...
        $new_id = Mullion_Campaign_Duplicator::duplicate(
            $source_id,
            $new_name,
            $copy_media,
    ...
    public static function batch_campaigns($request) {
        ...
        if ($action === 'delete') {
            ...
            foreach ($ids as $raw_id) {
                ...
                Mullion_DB::delete_access_requests_for_campaign($post_id);
                if ($purge_analytics) {
                    $wpdb->delete(Mullion_DB::get_analytics_table(), ['campaign_id' => $post_id], ['%d']); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
                }
                $deleted = wp_delete_post($post_id, true);
                if ($deleted) {
    ...
    public static function list_audit($request) {
        ...
        $db_count = Mullion_DB::list_audit_entries(['campaign_id' => $post_id, 'per_page' => 1, 'page' => 1])['total'];
        if ($db_count === 0) {
            $legacy = get_post_meta($post_id, 'audit_log', true);
            if (is_array($legacy) && count($legacy) > 0) {
                Mullion_DB::backfill_audit_entries($post_id, $legacy);
            }
        }

        // P28-F: pagination; P28-G: from/to/action filters.
        [$page, $per_page, ] = self::parse_pagination($request);
        $result = Mullion_DB::list_audit_entries([
            'campaign_id' => $post_id,
    ...
    public static function list_global_audit($request) {
        ...
        $result = Mullion_DB::list_audit_entries($args);
    ...
    public static function export_audit_log_binary($request) {
        ...
        $job_id = Mullion_Export_Engine::create_job('audit', $manifest, $media_items, required_tier: Mullion_Permissions::TIER_SYSTEM_ADMIN);
    ...
    private static function apply_campaign_meta($post_id, $request) {
        ...
        if (!empty($status)) {
            ...
            // are requested here.
            Mullion_Campaign_Status::set($post_id, $status);
        }
        if (is_array($tags)) {
