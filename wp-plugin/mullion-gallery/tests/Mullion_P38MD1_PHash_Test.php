...
class Mullion_P38MD1_PHash_Test extends WP_UnitTestCase {
    ...
    public function test_hamming_distance_zero_for_identical_hashes() {
        ...
        $this->assertEquals(0, Mullion_PHash::hamming_distance($hash, $hash));
    }
    ...
    public function test_hamming_distance_counts_differing_bits() {
        ...
        $a = 'f000000000000000';
        $b = '0000000000000000';
        $this->assertEquals(4, Mullion_PHash::hamming_distance($a, $b));
    }

    public function test_hamming_distance_returns_max_int_for_length_mismatch() {
        $this->assertEquals(PHP_INT_MAX, Mullion_PHash::hamming_distance('abc', 'abcd'));
    }

    public function test_is_image_mime_true_for_image_types() {
        $this->assertTrue(Mullion_PHash::is_image_mime('image/jpeg'));
        $this->assertTrue(Mullion_PHash::is_image_mime('image/png'));
        $this->assertTrue(Mullion_PHash::is_image_mime('image/gif'));
        $this->assertTrue(Mullion_PHash::is_image_mime('image/webp'));
    }

    public function test_is_image_mime_false_for_video_types() {
        $this->assertFalse(Mullion_PHash::is_image_mime('video/mp4'));
        $this->assertFalse(Mullion_PHash::is_image_mime('video/webm'));
    }
    ...
    public function test_get_campaigns_for_attachment_id_returns_correct_campaigns() {
        ...
        $campaigns = Mullion_DB::get_campaigns_for_attachment_id((int) $attachment_id);
        ...
        $none = Mullion_DB::get_campaigns_for_attachment_id(999999);
    ...
    public function test_video_upload_does_not_store_phash() {
        ...
        // that Mullion_PHash::is_image_mime() returns false for video types, which is
        // the gate used in upload_single_media_file().
        $this->assertFalse(
            Mullion_PHash::is_image_mime('video/mp4'),
            'Video MIME types must be excluded from pHash computation'
        );
        $this->assertFalse(
            Mullion_PHash::is_image_mime('video/webm'),
            'Video MIME types must be excluded from pHash computation'
        );
    }
}
