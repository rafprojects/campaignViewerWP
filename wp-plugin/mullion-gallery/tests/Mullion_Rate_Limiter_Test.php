<?php
/**
 * Tests for Mullion_Rate_Limiter class.
 *
 * @package Mullion
 */

class Mullion_Rate_Limiter_Test extends WP_UnitTestCase {
    ...
    private function transient_key( string $ip, string $endpoint ): string {
        return Mullion_Rate_Limiter::TRANSIENT_PREFIX . $endpoint . '_' . md5( $ip );
    }
    ...
    public function test_check_allows_first_request() {
        $result = Mullion_Rate_Limiter::check( $this->test_ip, $this->endpoint );

        $this->assertTrue( $result['allowed'] );
        $this->assertEquals( Mullion_Rate_Limiter::DEFAULT_LIMIT - 1, $result['remaining'] );
        $this->assertArrayNotHasKey( 'retry_after', $result );
    }

    public function test_check_remaining_decrements_on_subsequent_calls() {
        Mullion_Rate_Limiter::check( $this->test_ip, $this->endpoint ); // 1st
        $result = Mullion_Rate_Limiter::check( $this->test_ip, $this->endpoint ); // 2nd

        $this->assertTrue( $result['allowed'] );
        $this->assertEquals( Mullion_Rate_Limiter::DEFAULT_LIMIT - 2, $result['remaining'] );
    }
    ...
    public function test_check_blocks_when_limit_exceeded() {
        ...
        for ( $i = 0; $i < $limit; $i++ ) {
            Mullion_Rate_Limiter::check( $this->test_ip, $this->endpoint );
        }
    ...
    public function test_check_isolates_different_ips() {
        ...
        $result_b = Mullion_Rate_Limiter::check( '10.0.0.2', $this->endpoint );
    ...
    public function test_check_isolates_different_endpoints() {
        ...
        $blocked = Mullion_Rate_Limiter::check( $this->test_ip, 'endpoint_a' );
        $this->assertFalse( $blocked['allowed'] );

        // Endpoint B should still be fine.
        $result_b = Mullion_Rate_Limiter::check( $this->test_ip, 'endpoint_b' );
        $this->assertTrue( $result_b['allowed'] );
    ...
    public function test_default_public_limit_triggers_at_threshold() {
        ...
        // Exhaust the allowance.
        for ( $i = 0; $i < $limit; $i++ ) {
            Mullion_Rate_Limiter::check( $this->test_ip, 'public' );
        }

        // Next call should be blocked.
        $result = Mullion_Rate_Limiter::check( $this->test_ip, 'public' );
    ...
    public function test_rate_limit_requests_per_minute_in_settings_defaults() {
        $defaults = Mullion_Settings_Registry::get_defaults();
    ...
    public function test_rate_limit_requests_per_minute_is_space_overridable() {
        $fields = Mullion_Settings_Registry::get_space_overridable_fields();
    ...
    public function test_rate_limit_requests_per_minute_sanitization() {
        $sanitized = Mullion_Settings::sanitize_settings( [ 'rate_limit_requests_per_minute' => '300' ] );
        $this->assertSame( 300, $sanitized['rate_limit_requests_per_minute'] );

        // Clamps at 6000.
        $sanitized = Mullion_Settings::sanitize_settings( [ 'rate_limit_requests_per_minute' => 99999 ] );
        $this->assertSame( 6000, $sanitized['rate_limit_requests_per_minute'] );

        // Negative clamps to 0.
        $sanitized = Mullion_Settings::sanitize_settings( [ 'rate_limit_requests_per_minute' => -5 ] );
