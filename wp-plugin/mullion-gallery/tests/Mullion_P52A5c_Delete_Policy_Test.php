 ...
 */
class Mullion_P52A5c_Delete_Policy_Test extends WP_UnitTestCase {
    ...
    private function make_template(): string {
        $tpl = Mullion_Layout_Templates::create(['name' => 'A5c Template', 'canvasAspectRatio' => 1.5]);
        $this->assertIsArray($tpl, 'fixture template must be created');
        return $tpl['id'];
    }

    private function make_asset(): string {
        $asset = Mullion_Asset_Library::add(['url' => 'https://example.com/a5c.svg', 'name' => 'A5c Asset']);
    ...
    public function test_template_delete_force_overrides_in_use() {
        ...
        $this->assertNull(Mullion_Layout_Templates::get($tid), 'template must be gone after a forced delete');
    }
