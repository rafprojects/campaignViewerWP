 ...
 */
class Mullion_P50B_Space_Library_Test extends WP_UnitTestCase {

    /**
     * test_migration_backfills_existing_assets_into_delegated_spaces calls
     * Mullion_DB::maybe_upgrade() which runs dbDelta() (DDL) → implicit MySQL
    ...
    public function test_non_owner_cannot_modify_delegated_space_library() {
        ...
        $this->assertSame([], Mullion_DB::get_space_library_assets($space, 'asset'));
    }
    ...
    public function test_migration_backfills_existing_assets_into_delegated_spaces() {
        ...
        $this->assertContains($font, Mullion_DB::get_space_library_assets($delegated, 'font'));
        $this->assertSame([], Mullion_DB::get_space_library_assets($open, 'asset'), 'Open spaces need no association rows.');
    }
}
