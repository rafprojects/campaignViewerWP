 ...
 * @package Mullion
 */
class Mullion_P63A_B_Rest_Rate_Limit_Test extends WP_UnitTestCase {
    ...
    public function test_transient_backend_throttles_without_persistent_cache() {
        ...
        $r1 = Mullion_REST_Base::rate_limit_public( $request );
        $r2 = Mullion_REST_Base::rate_limit_public( $request );
        $r3 = Mullion_REST_Base::rate_limit_public( $request );
    ...
    public function test_buckets_per_client_ip_behind_trusted_proxy() {
        ...
        $a1 = Mullion_REST_Base::rate_limit_public( $request );
        $a2 = Mullion_REST_Base::rate_limit_public( $request );

        // Client B behind the SAME proxy: must have an independent bucket.
        $_SERVER['HTTP_X_FORWARDED_FOR'] = '203.0.113.22';
        $b1 = Mullion_REST_Base::rate_limit_public( $request );
