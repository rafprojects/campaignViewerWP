 ...
 *
 * Asserts the Mullion_Permissions action→strategy matrix and that EVERY registered
 ...
 *   - Mullion_P33C_Role_Enforcement_Test  (require_campaign_space_access — per-campaign mutation is role+space-scoped)
 *   - Mullion_P47_Spaces_Isolation_Test   (space-scoped read/isolation, delegated mode)
 * with the require_admin / public / authenticated strategies pinned below.
 */
class Mullion_P52A_Permission_Matrix_Test extends WP_UnitTestCase {

    /**
     * The authoritative matrix, hand-maintained INDEPENDENTLY of Mullion_Permissions::MAP
    ...
    public function test_map_matches_frozen_matrix() {
        ...
        $this->assertCount(
            count($expected),
            Mullion_Permissions::MAP,
            'Mullion_Permissions::MAP size drifted from the frozen P52-A matrix'
        );
        ...
        foreach ($expected as $action => $strategy) {
            $this->assertSame(
                $strategy,
                Mullion_Permissions::strategy($action),
                "Strategy for action '{$action}' drifted from the frozen matrix"
            );
        }

        // No undocumented actions snuck into the map.
        foreach (Mullion_Permissions::MAP as $action => $strategy) {
            $this->assertArrayHasKey(
                $action,
                $expected,
                "Mullion_Permissions::MAP has action '{$action}' not present in the frozen matrix"
            );
        }
    }

    public function test_all_strategies_are_callable_primitives() {
        $strategies = array_unique(array_values(Mullion_Permissions::MAP));
        foreach ($strategies as $strategy) {
            if ($strategy === '__return_true') {
                continue;
            }
            $this->assertTrue(
                is_callable([Mullion_REST_Base::class, $strategy]),
                "Strategy '{$strategy}' is not a callable Mullion_REST_Base primitive"
            );
        }
    }
    ...
    public function test_every_registered_route_resolves_through_permissions() {
        ...
        foreach ($routes as $route => $endpoints) {
            ...
            foreach ($endpoints as $endpoint) {
                ...
                $this->assertInstanceOf(
                    Closure::class,
                    $pc,
                    "Route {$route} permission_callback is not a Mullion_Permissions gate (got " . gettype($pc) . ')'
                );
                ...
                $this->assertSame(
                    'Mullion_Permissions',
                    $scope->getName(),
                    "Route {$route} permission_callback does not originate from Mullion_Permissions"
                );

                $vars = $ref->getStaticVariables();
                $this->assertArrayHasKey('action', $vars, "Route {$route} gate has no bound action");
                $this->assertTrue(
                    Mullion_Permissions::has($vars['action']),
        ...
        $this->assertEqualsCanonicalizing(
            array_keys(Mullion_Permissions::MAP),
            array_keys($found_actions),
            'The set of actions wired to routes must equal the Mullion_Permissions map exactly'
        );
    }
    ...
    public function test_always_allow_strategy() {
        wp_set_current_user(0);
        $this->assertTrue(Mullion_Permissions::check('auth.login'), '__return_true action must always allow');
        $this->assertTrue(Mullion_Permissions::check('layout_templates.read_public'), 'public template read must allow');
    }

    public function test_require_authenticated_strategy() {
        wp_set_current_user(0);
        $this->assertFalse(Mullion_Permissions::check('auth.permissions.read'), 'anonymous must fail require_authenticated');

        $uid = self::factory()->user->create(['role' => 'subscriber']);
        wp_set_current_user($uid);
        $this->assertTrue(Mullion_Permissions::check('auth.permissions.read'), 'logged-in user must pass require_authenticated');
    }
    ...
    public function test_unknown_action_fails_closed() {
        $this->make_admin();
        $this->assertFalse(
            Mullion_Permissions::check('does.not.exist'),
            'unknown actions must fail closed even for an administrator'
        );
        $this->assertFalse(Mullion_Permissions::has('does.not.exist'));
        $this->assertSame('', Mullion_Permissions::strategy('does.not.exist'));
    }

    public function test_gate_returns_callable_matching_check() {
        $this->make_admin();
        $gate = Mullion_Permissions::gate('settings.update');
    ...
    public function test_gate_warns_on_unknown_action() {
        $this->setExpectedIncorrectUsage('Mullion_Permissions::gate');
        Mullion_Permissions::gate('totally.bogus.action');
    }
    ...
    public function test_actor_has_tier_resolves_current_user() {
        ...
        $this->assertFalse(Mullion_Permissions::actor_has_tier(Mullion_Permissions::TIER_SYSTEM_ADMIN));
        $this->assertFalse(Mullion_Permissions::actor_has_tier(Mullion_Permissions::TIER_EDITOR));
        $this->assertTrue(Mullion_Permissions::actor_has_tier(Mullion_Permissions::TIER_VIEWER));
        ...
        $this->assertFalse(Mullion_Permissions::actor_has_tier(Mullion_Permissions::TIER_SYSTEM_ADMIN));
        $this->assertFalse(Mullion_Permissions::actor_has_tier(Mullion_Permissions::TIER_EDITOR));
        $this->assertFalse(Mullion_Permissions::actor_has_tier(Mullion_Permissions::TIER_VIEWER));
        $this->assertFalse(Mullion_Permissions::actor_has_tier('nonexistent_tier'));
    }
}
