<?php
/**
 * P65-A — Mullion_Campaign_IO service tests.
 *
...
class Mullion_P65A_Campaign_IO_Test extends WP_UnitTestCase {
    ...
    public function test_json_import_normalizes_datetimes() {
        ...
        $result = Mullion_Campaign_IO::import_entry($entry, null, ['via' => 'cli', 'format' => 'json']);
    ...
    public function test_service_round_trips_layout_template_end_to_end() {
        ...
        $result = Mullion_Campaign_IO::import_entry(
    ...
    public function test_stream_zip_entry_to_file_copies_bytes() {
        ...
        $ok = Mullion_Export_Engine::stream_zip_entry_to_file($zip, 'media/hello.txt', $dest);
        $this->assertTrue($ok);
        $this->assertSame('streamed-content', file_get_contents($dest));

        $missing = Mullion_Export_Engine::stream_zip_entry_to_file($zip, 'media/nope.txt', $dest);
