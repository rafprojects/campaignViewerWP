...
class Mullion_Thumbnail_Cache_Test extends WP_UnitTestCase {
    ...
    public function setUp(): void {
        ...
        $this->cache_dir = Mullion_Thumbnail_Cache::get_cache_dir();
    }
    ...
    public function test_get_cache_dir_returns_path_under_uploads() {
        $dir = Mullion_Thumbnail_Cache::get_cache_dir();
        $this->assertIsString($dir);
        $this->assertStringContainsString(Mullion_Thumbnail_Cache::UPLOAD_DIR, $dir);
    }
    ...
    public function test_get_cache_url_returns_url_string() {
        $url = Mullion_Thumbnail_Cache::get_cache_url();
        $this->assertIsString($url);
        $this->assertStringContainsString(Mullion_Thumbnail_Cache::UPLOAD_DIR, $url);
    }

    // ── cache_thumbnail ────────────────────────────────────────────────────────

    public function test_cache_thumbnail_rejects_empty_url() {
        $result = Mullion_Thumbnail_Cache::cache_thumbnail('', 'https://example.com/media/1');
        $this->assertFalse($result['cached']);
        $this->assertStringContainsString('Invalid', $result['error']);
    }

    public function test_cache_thumbnail_rejects_invalid_url() {
        $result = Mullion_Thumbnail_Cache::cache_thumbnail('not-a-url', 'https://example.com/media/1');
        $this->assertFalse($result['cached']);
    }
    ...
    public function test_cache_thumbnail_stores_metadata_on_success() {
        ...
        $result = Mullion_Thumbnail_Cache::cache_thumbnail(
    ...
    public function test_get_cached_url_returns_null_for_uncached() {
        $result = Mullion_Thumbnail_Cache::get_cached_url('https://example.com/never-cached');
    ...
    public function test_get_cached_url_returns_url_for_new_format_entry() {
        ...
        $path     = $dir . '/' . $hash . '.jpg';
        $expected = Mullion_Thumbnail_Cache::get_cache_url() . '/' . $hash . '.jpg';
        ...
        $result = Mullion_Thumbnail_Cache::get_cached_url($source);
    ...
    public function test_get_cached_url_returns_null_for_expired_entry() {
        ...
        $this->assertNull(Mullion_Thumbnail_Cache::get_cached_url($source));
    }
    ...
    public function test_cache_campaign_thumbnails_returns_zero_for_no_media() {
        ...
        $result = Mullion_Thumbnail_Cache::cache_campaign_thumbnails($cid);
    ...
    public function test_cleanup_expired_removes_old_entries() {
        ...
        $this->write_thumb_entry($hash, [
            'local_path' => $path,
            'cached_at'  => time() - (Mullion_Thumbnail_Cache::DEFAULT_TTL * 3),
            'file_size'  => 8,
        ]);
