...
class Mullion_DB_Test extends WP_UnitTestCase {
    ...
    public function test_sync_media_refs_inserts_rows() {
        ...
        Mullion_DB::sync_media_refs($cid, [
            ['id' => 'media-1'],
            ['id' => 'media-2'],
        ]);

        $usage = Mullion_DB::get_media_usage('media-1');
    ...
    public function test_sync_media_refs_deduplicates_ids() {
        ...
        $summary = Mullion_DB::get_media_usage_summary(['dup-1']);
    ...
    public function test_sync_media_refs_skips_empty_ids() {
        ...
        global $wpdb;
        $count = $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(*) FROM " . Mullion_DB::get_media_refs_table() . " WHERE campaign_id = %d", $cid)
        );
    ...
    public function test_sync_media_refs_replaces_previous_refs() {
        ...
        $this->assertCount(1, Mullion_DB::get_media_usage('old-1'));
        ...
        $this->assertCount(0, Mullion_DB::get_media_usage('old-1'));
        $this->assertCount(0, Mullion_DB::get_media_usage('old-2'));
        $this->assertCount(1, Mullion_DB::get_media_usage('new-1'));
    }
    ...
    public function test_get_media_usage_returns_campaign_title() {
        ...
        Mullion_DB::sync_media_refs($cid, [['id' => 'title-check']]);

        $usage = Mullion_DB::get_media_usage('title-check');
        $this->assertEquals('My Gallery', $usage[0]['title']);
    }

    public function test_get_media_usage_returns_empty_for_unknown_id() {
        $this->create_refs_table();
        $usage = Mullion_DB::get_media_usage('nonexistent');
    ...
    public function test_get_media_usage_across_multiple_campaigns() {
        ...
        Mullion_DB::sync_media_refs($c1, [['id' => 'shared']]);
        Mullion_DB::sync_media_refs($c2, [['id' => 'shared']]);

        $usage = Mullion_DB::get_media_usage('shared');
    ...
    public function test_get_media_usage_summary_returns_counts() {
        ...
        Mullion_DB::sync_media_refs($c1, [['id' => 'a'], ['id' => 'b']]);
        Mullion_DB::sync_media_refs($c2, [['id' => 'a']]);

        $summary = Mullion_DB::get_media_usage_summary(['a', 'b', 'c']);
    ...
    public function test_get_media_usage_summary_handles_empty_array() {
        $this->create_refs_table();
        $summary = Mullion_DB::get_media_usage_summary([]);
    ...
    public function test_delete_media_refs_removes_only_target_campaign() {
        ...
        Mullion_DB::sync_media_refs($c1, [['id' => 'x']]);
        Mullion_DB::sync_media_refs($c2, [['id' => 'x']]);

        Mullion_DB::delete_media_refs($c1);

        $usage = Mullion_DB::get_media_usage('x');
        $this->assertCount(1, $usage);
    ...
    public function test_delete_media_refs_noop_for_unknown_campaign() {
        $this->create_refs_table();
        // Should not throw.
        Mullion_DB::delete_media_refs(99999);
    ...
    public function test_backfill_populates_refs_from_existing_campaigns() {
        ...
        $usage = Mullion_DB::get_media_usage('bf-1');
        $this->assertCount(1, $usage);
        $this->assertEquals(strval($cid), $usage[0]['id']);

        $usage2 = Mullion_DB::get_media_usage('bf-2');
    ...
    public function test_insert_and_get_access_request() {
        ...
        Mullion_DB::insert_access_request([
            'token'        => $token,
            'campaign_id'  => 42,
        ...
        $row = Mullion_DB::get_access_request($token);
    ...
    public function test_get_access_request_returns_null_for_unknown_token() {
        $this->create_access_requests_table();
        $this->assertNull(Mullion_DB::get_access_request('nonexistent'));
    }
    ...
    public function test_update_access_request_status() {
        ...
        Mullion_DB::update_access_request_status($token, 'approved');
    ...
    public function test_delete_access_request() {
        ...
        Mullion_DB::delete_access_request($token);
        $this->assertNull(Mullion_DB::get_access_request($token));
    }
    ...
    public function test_list_access_requests_returns_campaign_rows() {
        ...
        $rows = Mullion_DB::list_access_requests($cid);
    ...
    public function test_list_access_requests_filters_by_status() {
        ...
        Mullion_DB::update_access_request_status($tokenB, 'approved');

        $pending = Mullion_DB::list_access_requests($cid, 'pending');
        $this->assertCount(1, $pending);
        $this->assertEquals('a@e.com', $pending[0]['email']);

        $approved = Mullion_DB::list_access_requests($cid, 'approved');
    ...
    public function test_find_access_request_by_email() {
        ...
        $row = Mullion_DB::find_access_request_by_email('find@example.com', 5);
        $this->assertNotNull($row);

        // Different campaign — no match.
        $none = Mullion_DB::find_access_request_by_email('find@example.com', 999);
    ...
    public function test_delete_access_requests_for_campaign() {
        ...
        Mullion_DB::delete_access_requests_for_campaign(1);

        $this->assertCount(0, Mullion_DB::list_access_requests(1));
        $this->assertCount(1, Mullion_DB::list_access_requests(2));
    }
    ...
    public function test_migrate_access_requests_from_options() {
        ...
        $row2 = Mullion_DB::get_access_request($token2);
