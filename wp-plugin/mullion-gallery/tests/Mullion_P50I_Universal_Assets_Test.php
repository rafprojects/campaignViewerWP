 ...
 *  - Mullion_Asset_Library::set_universal() round-trips and get_all() reflects it.
 ...
 */
class Mullion_P50I_Universal_Assets_Test extends WP_UnitTestCase {

    /**
     * The migration test calls Mullion_DB::maybe_upgrade() which runs DDL → an
    ...
    public function test_set_universal_round_trip() {
        ...
        $this->assertTrue(Mullion_Asset_Library::set_universal($id, false));
    ...
    public function test_set_universal_unknown_overlay_returns_false() {
        $this->set_super_admin();
        $this->assertFalse(Mullion_Asset_Library::set_universal('00000000-0000-0000-0000-000000000000', true));
    }

    private function overlay_is_universal(string $id): bool {
        foreach ( Mullion_Asset_Library::get_all() as $item ) {
            if ( ($item['id'] ?? '') === $id ) {
