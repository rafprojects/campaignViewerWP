...
/**
 * PHPUnit coverage for Mullion_Logger — structured logging facade (P32-D).
 *
 * Validates:
 ...
 *  - That Mullion_Monitoring::log_fatal_error() now routes through the logger and
 *    produces a structured record in the ring buffer.
 */
class Mullion_Logger_Test extends WP_UnitTestCase {

    public function setUp(): void {
        parent::setUp();
        Mullion_Logger::clear_logs();
    }
    ...
    public function test_info_stores_record_with_correct_schema(): void {
        Mullion_Logger::info('rest', 'Test info message', ['key' => 'value']);

        $logs = Mullion_Logger::get_recent_logs(1);
    ...
    public function test_warning_stores_record_with_warning_level(): void {
        Mullion_Logger::warning('oembed', 'oEmbed failed', ['url' => 'https://example.com']);
    ...
    public function test_error_stores_record_with_error_level(): void {
        Mullion_Logger::error('monitoring', 'Fatal PHP error', ['type' => E_ERROR]);
    ...
    public function test_timestamp_is_utc_iso8601(): void {
        Mullion_Logger::info('test', 'Timestamp format check');
    ...
    public function test_empty_data_omits_data_key(): void {
        Mullion_Logger::info('test', 'No data payload');
    ...
    public function test_records_are_newest_first(): void {
        Mullion_Logger::info('test', 'First');
        Mullion_Logger::info('test', 'Second');
        Mullion_Logger::info('test', 'Third');

        $logs = Mullion_Logger::get_recent_logs(3);
    ...
    public function test_clear_logs_empties_the_buffer(): void {
        Mullion_Logger::warning('test', 'Before clear');
        $this->assertNotEmpty(Mullion_Logger::get_recent_logs());

        Mullion_Logger::clear_logs();
        $this->assertEmpty(Mullion_Logger::get_recent_logs());
    }

    // ── monitoring integration ─────────────────────────────────────────────

    /**
     * Mullion_Monitoring::log_fatal_error() now routes through Mullion_Logger.
    ...
    public function test_get_health_data_includes_recent_logs_key(): void {
        Mullion_Logger::warning('rest', 'Slow REST request detected', ['elapsedMs' => 1200]);
