 ...
 */
class Mullion_P39IN1_Webhook_Test extends WP_UnitTestCase {
    ...
    public function test_get_endpoints_returns_empty_array_by_default() {
        $this->assertSame([], Mullion_Webhooks::get_endpoints());
    }
    ...
    public function test_save_and_get_endpoints_roundtrip() {
        ...
        $this->assertCount(1, Mullion_Webhooks::get_endpoints());
        $this->assertSame('https://example.com/hook', Mullion_Webhooks::get_endpoints()[0]['url']);
    }
    ...
    public function test_save_endpoints_reindexes() {
        ...
        Mullion_Webhooks::save_endpoints($endpoints);
        $stored = Mullion_Webhooks::get_endpoints();
    ...
    public function test_generate_secret_is_unique() {
        $a = Mullion_Webhooks::generate_secret();
        $b = Mullion_Webhooks::generate_secret();
        $this->assertNotSame($a, $b);
    }

    public function test_mask_secret_hides_all_but_last_8() {
        $secret = '1234567890abcdef';
        $masked = Mullion_Webhooks::mask_secret($secret);
    ...
    public function test_sign_produces_sha256_prefix() {
        $sig = Mullion_Webhooks::sign('payload', 'mysecret');
        $this->assertStringStartsWith('sha256=', $sig);
    }

    public function test_sign_is_deterministic() {
        $a = Mullion_Webhooks::sign('body', 'secret');
        $b = Mullion_Webhooks::sign('body', 'secret');
        $this->assertSame($a, $b);
    }

    public function test_sign_differs_for_different_secrets() {
        $a = Mullion_Webhooks::sign('body', 'secret1');
        $b = Mullion_Webhooks::sign('body', 'secret2');
        $this->assertNotSame($a, $b);
    }
    ...
    public function test_sanitize_url_accepts_https() {
        $url = Mullion_Webhooks::sanitize_url('https://hooks.example.com/path');
        $this->assertNotEmpty($url);
    }

    public function test_sanitize_url_accepts_http() {
        $url = Mullion_Webhooks::sanitize_url('http://hooks.example.com/path');
        $this->assertNotEmpty($url);
    }

    public function test_sanitize_url_rejects_ftp() {
        $url = Mullion_Webhooks::sanitize_url('ftp://files.example.com/hook');
        $this->assertSame('', $url);
    }

    public function test_sanitize_url_rejects_empty() {
        $this->assertSame('', Mullion_Webhooks::sanitize_url(''));
    }

    public function test_sanitize_url_rejects_no_host() {
        $this->assertSame('', Mullion_Webhooks::sanitize_url('https://'));
    }

    // ── Event sanitization ─────────────────────────────────────────────────────

    public function test_sanitize_events_filters_invalid() {
        $events = Mullion_Webhooks::sanitize_events(['campaign.created', 'not.a.valid.event', 'media.added']);
        $this->assertSame(['campaign.created', 'media.added'], $events);
    }

    public function test_sanitize_events_returns_empty_for_empty_input() {
        $this->assertSame([], Mullion_Webhooks::sanitize_events([]));
    }
    ...
    public function test_format_endpoint_masks_secret() {
        ...
        $formatted = Mullion_Webhooks::format_endpoint_for_api(0, $endpoint);
    ...
    public function test_format_endpoint_includes_index() {
        ...
        $formatted = Mullion_Webhooks::format_endpoint_for_api(3, $endpoint);
    ...
    public function test_get_delivery_log_returns_empty_array_by_default() {
        $this->assertSame([], Mullion_Webhooks::get_delivery_log());
    }
    ...
    public function test_dispatch_logs_delivery_on_success() {
        ...
        Mullion_Webhooks::save_endpoints([[
        ...
        Mullion_Webhooks::dispatch('campaign.created', ['id' => 1, 'title' => 'Test']);
    ...
    public function test_dispatch_logs_delivery_on_failure() {
        ...
        Mullion_Webhooks::dispatch('campaign.archived', ['id' => 1]);
    ...
    public function test_dispatch_defers_first_attempt_to_cron_for_endpoint_with_id() {
        ...
        Mullion_Webhooks::dispatch('campaign.created', ['id' => 7, 'title' => 'Async']);

        // The originating request did NOT deliver inline — nothing logged yet.
        $this->assertEmpty(
            Mullion_Webhooks::get_delivery_log(),
        ...
        foreach (_get_cron_array() as $events) {
            if (isset($events[Mullion_Webhooks::RETRY_HOOK])) {
                $hook = reset($events[Mullion_Webhooks::RETRY_HOOK]);
        ...
        Mullion_Webhooks::retry_delivery(...$found_args);
    ...
    public function test_dispatch_skips_disabled_endpoint() {
        ...
        Mullion_Webhooks::dispatch('campaign.created', ['id' => 1]);

        remove_all_filters('pre_http_request');

        $this->assertEmpty(Mullion_Webhooks::get_delivery_log());
    }
    ...
    public function test_dispatch_empty_event_filter_receives_all_events() {
        ...
        Mullion_Webhooks::dispatch('media.removed', ['campaignId' => 1]);
        Mullion_Webhooks::dispatch('access.revoked', ['campaignId' => 1]);

        remove_all_filters('pre_http_request');

        $this->assertCount(2, Mullion_Webhooks::get_delivery_log());
    }
    ...
    public function test_delivery_log_capped_at_max_entries() {
        ...
        for ($i = 0; $i < Mullion_Webhooks::MAX_LOG + 10; $i++) {
            Mullion_Webhooks::dispatch('campaign.archived', ['id' => $i]);
        }

        remove_all_filters('pre_http_request');

        $log = Mullion_Webhooks::get_delivery_log();
        $this->assertCount(Mullion_Webhooks::MAX_LOG, $log);
    }
    ...
    public function test_delivery_payload_contains_event_and_timestamp() {
        ...
        Mullion_Webhooks::dispatch('campaign.created', ['id' => 42, 'title' => 'My Campaign']);
    ...
    public function test_delivery_includes_signature_header() {
        ...
        Mullion_Webhooks::dispatch('media.added', ['campaignId' => 1]);
    ...
    public function test_rest_create_endpoint_enforces_max_limit() {
        wp_set_current_user($this->factory->user->create(['role' => 'administrator']));
        $endpoints = [];
        for ($i = 0; $i < Mullion_Webhooks::MAX_ENDPOINTS; $i++) {
    ...
    public function test_rest_rotate_secret_returns_new_secret() {
        wp_set_current_user($this->factory->user->create(['role' => 'administrator']));
        $original = Mullion_Webhooks::generate_secret();
        Mullion_Webhooks::save_endpoints([[
        ...
        $this->assertSame($new_secret, Mullion_Webhooks::get_endpoints()[0]['secret']);
    }
