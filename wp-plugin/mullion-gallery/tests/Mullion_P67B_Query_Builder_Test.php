 ...
 * Mullion_Campaign_Controller::list_campaigns() — build_campaign_query_args() and
 * build_campaign_cache_key(). These pin the WP_Query args and cache keys so the
 * extraction is provably behavior-preserving.
 */
class Mullion_P67B_Query_Builder_Test extends WP_UnitTestCase {
    ...
    private function build_args(array $filters, bool $is_admin, int $user_id): array {
        $m = new ReflectionMethod('Mullion_Campaign_Controller', 'build_campaign_query_args');
    ...
    private function build_key(array $filters, int $user_id, bool $is_admin): string {
        $m = new ReflectionMethod('Mullion_Campaign_Controller', 'build_campaign_cache_key');
        $m->setAccessible(true);
    ...
    public function test_admin_args_have_no_scoping_or_schedule_window() {
        ...
        $this->assertSame(Mullion_Campaign_Templates::META_IS_TEMPLATE, $args['meta_query'][0]['key']);
