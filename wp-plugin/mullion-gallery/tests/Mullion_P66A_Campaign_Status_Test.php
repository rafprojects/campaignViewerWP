...
/**
 * P66-A: Mullion_Campaign_Status — the single source of truth for campaign status
 ...
 */
class Mullion_P66A_Campaign_Status_Test extends WP_UnitTestCase {
    ...
    public function test_set_accepts_each_valid_status() {
        ...
        foreach ( ['draft', 'active', 'archived'] as $status ) {
            $this->assertTrue(Mullion_Campaign_Status::set($id, $status));
    ...
    public function test_restore_stamps_restored_at_and_clears_archived_at() {
        ...
        Mullion_Campaign_Status::set($id, 'active');
    ...
    public function test_set_without_ctx_fires_no_side_effects() {
        ...
        $this->assertSame($cache_before, Mullion_REST::get_cache_version(), 'Cache must not bump without ctx[cache]');
        $audit = Mullion_DB::list_audit_entries(['campaign_id' => $id, 'per_page' => 10, 'page' => 1]);
    ...
    public function test_set_with_ctx_fires_audit_hook_and_cache() {
        ...
        $this->assertSame($cache_before + 1, Mullion_REST::get_cache_version(), 'ctx[cache] must bump the cache version once');
        $audit = Mullion_DB::list_audit_entries(['campaign_id' => $id, 'per_page' => 10, 'page' => 1]);
    ...
    public function test_stamp_archived_batch_stamps_all_and_clears_restored_at() {
        ...
        $count = Mullion_Campaign_Status::stamp_archived_batch([$a, $b, 0, -3]);
    ...
    public function test_stamp_archived_batch_writes_exactly_one_row() {
        ...
        Mullion_Campaign_Status::stamp_archived_batch([$a]);
