 ...
 *
 * Covers:
 *  - Mullion_Asset_Library::add() stores tags; get_all() returns them as string[].
...
class Mullion_P50K_Asset_Tags_Test extends WP_UnitTestCase {
    ...
    public function test_set_tags_round_trip() {
        ...
        $this->assertTrue(Mullion_Asset_Library::set_tags($id, ['badge', '  spaced  ', '']));
        // sanitize_text_field trims; empties dropped.
        $this->assertEqualsCanonicalizing(['badge', 'spaced'], $this->get_asset($id)['tags']);

        $this->assertTrue(Mullion_Asset_Library::set_tags($id, []));
        $this->assertSame([], $this->get_asset($id)['tags']);
    }

    public function test_set_tags_unknown_asset_returns_false() {
        $this->set_super_admin();
        $this->assertFalse(Mullion_Asset_Library::set_tags('00000000-0000-0000-0000-000000000000', ['x']));
    }
