 ...
 *
 * Exercises the public wrapper Mullion_REST_Base::check_private_ip() directly (no DNS)
 * to assert the newly-covered ranges are blocked and that no regression was
 * introduced on adjacent public space.
 *
 * @package Mullion
 */
class Mullion_P63F_Private_IP_Test extends WP_UnitTestCase {
    ...
    public function test_newly_covered_ipv4_ranges_are_private( string $ip ) {
        $this->assertTrue(
            Mullion_REST_Base::check_private_ip( $ip ),
    ...
    public function test_nat64_wrapping_link_local_metadata_is_private() {
        ...
        $this->assertTrue( Mullion_REST_Base::check_private_ip( '64:ff9b::a9fe:a9fe' ) );
    }

    public function test_nat64_wrapping_rfc1918_is_private() {
        // 64:ff9b::10.0.0.1  → embedded 10.0.0.1 (0a00:0001).
        $this->assertTrue( Mullion_REST_Base::check_private_ip( '64:ff9b::a00:1' ) );
    }
    ...
    public function test_nat64_wrapping_public_ipv4_is_not_private() {
        ...
        $this->assertFalse( Mullion_REST_Base::check_private_ip( '64:ff9b::808:808' ) );
    }
    ...
    public function test_existing_ranges_still_private( string $ip ) {
        $this->assertTrue( Mullion_REST_Base::check_private_ip( $ip ), "$ip should still be private" );
    }
