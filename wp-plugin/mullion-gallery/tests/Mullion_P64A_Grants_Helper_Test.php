...
/**
 * P64-A: Mullion_Grants — the shared access-grant helper extracted from ~10
 ...
 */
class Mullion_P64A_Grants_Helper_Test extends WP_UnitTestCase {

    // ── upsert ────────────────────────────────────────────────────────────────

    public function test_upsert_appends_new_user() {
        $grants = [['userId' => 1, 'access_level' => 'viewer']];
        $out    = Mullion_Grants::upsert($grants, ['userId' => 2, 'access_level' => 'viewer']);
    ...
    public function test_upsert_replaces_existing_user_entry() {
        ...
        $out = Mullion_Grants::upsert($grants, ['userId' => 1, 'access_level' => 'viewer', 'grantedAt' => 'new']);
    ...
    public function test_upsert_treats_missing_user_id_as_zero() {
        $out = Mullion_Grants::upsert([['userId' => 0]], ['foo' => 'bar']);
    ...
    public function test_remove_drops_all_entries_for_user_and_reindexes() {
        ...
        $out = Mullion_Grants::remove($grants, 1);
    ...
    public function test_remove_noop_when_user_absent() {
        $grants = [['userId' => 5]];
        $this->assertSame($grants, Mullion_Grants::remove($grants, 99));
    }

    // ── is_expired ──────────────────────────────────────────────────────────────

    public function test_is_expired_false_for_absent_or_empty_expiry() {
        $this->assertFalse(Mullion_Grants::is_expired([]));
        $this->assertFalse(Mullion_Grants::is_expired(['expires_at' => null]));
        $this->assertFalse(Mullion_Grants::is_expired(['expires_at' => '']));
        $this->assertFalse(Mullion_Grants::is_expired(['expires_at' => '0']));
    }

    public function test_is_expired_true_for_past_expiry() {
        $past = gmdate('c', time() - 3600);
        $this->assertTrue(Mullion_Grants::is_expired(['expires_at' => $past]));
    }

    public function test_is_expired_false_for_future_expiry() {
        $future = gmdate('c', time() + 3600);
        $this->assertFalse(Mullion_Grants::is_expired(['expires_at' => $future]));
    }
    ...
    public function test_is_expired_true_for_unparseable_expiry() {
        ...
        $this->assertTrue(Mullion_Grants::is_expired(['expires_at' => 'not-a-date']));
    }

    public function test_is_expired_respects_injected_now() {
        $entry = ['expires_at' => gmdate('c', 1_000_000)];
        $this->assertTrue(Mullion_Grants::is_expired($entry, 2_000_000), 'expired relative to a later now');
        $this->assertFalse(Mullion_Grants::is_expired($entry, 500_000), 'not yet expired relative to an earlier now');
    }
    ...
    public function test_filter_active_keeps_only_non_expired() {
        ...
        $out = Mullion_Grants::filter_active($grants);
        $ids = array_column($out, 'userId');
    ...
    public function test_has_active_grant_true_for_non_expired_match() {
        ...
        $this->assertFalse(Mullion_Grants::has_active_grant($grants, 1));
        $this->assertTrue(Mullion_Grants::has_active_grant($grants, 2));
        $this->assertFalse(Mullion_Grants::has_active_grant($grants, 3), 'no entry for this user at all');
    }

    // ── parse_expiry_param ──────────────────────────────────────────────────────

    public function test_parse_expiry_param_null_and_empty_return_null() {
        $this->assertNull(Mullion_Grants::parse_expiry_param(null));
        $this->assertNull(Mullion_Grants::parse_expiry_param(''));
    }

    public function test_parse_expiry_param_normalises_valid_datetime() {
        $out = Mullion_Grants::parse_expiry_param('2030-06-01T12:00:00+00:00');
    ...
    public function test_validate_access_level_passes_accepted_tiers() {
        $this->assertSame('viewer', Mullion_Grants::validate_access_level('viewer'));
        $this->assertSame('editor', Mullion_Grants::validate_access_level('editor'));
        $this->assertSame('owner', Mullion_Grants::validate_access_level('owner'));
    }

    public function test_validate_access_level_defaults_junk_to_viewer() {
        $this->assertSame('viewer', Mullion_Grants::validate_access_level('superuser'));
        $this->assertSame('viewer', Mullion_Grants::validate_access_level(null));
        $this->assertSame('viewer', Mullion_Grants::validate_access_level(''));
    }
    ...
    public function test_enrich_users_attaches_user_and_normalises_level() {
        ...
        $out     = Mullion_Grants::enrich_users($entries);
    ...
    public function test_enrich_users_with_expiry_flag_toggles_is_expired() {
        ...
        $without = Mullion_Grants::enrich_users([$past], false);
        $this->assertArrayNotHasKey('is_expired', $without[0], 'company-access shape omits is_expired');

        $with = Mullion_Grants::enrich_users([$past], true);
