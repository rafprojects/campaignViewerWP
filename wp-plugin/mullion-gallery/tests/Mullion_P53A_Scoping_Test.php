 ...
 */
class Mullion_P53A_Scoping_Test extends WP_UnitTestCase {
    ...
    private function grant_space(int $space_id, int $user_id): void {
        Mullion_DB::update_space($space_id, [
    ...
    public function test_actor_space_access_for_editor() {
        ...
        $this->assertFalse(Mullion_REST_Base::current_actor_can_access_space($open), 'editor cannot access an ungranted open space');
        $this->assertFalse(Mullion_REST_Base::current_actor_can_access_space($deleg), 'editor cannot access an ungranted delegated space');
        $this->assertTrue(Mullion_REST_Base::current_actor_can_access_space($granted), 'editor accesses a space it was explicitly granted');
    }
    ...
    public function test_actor_space_access_for_system_admin() {
        ...
        $deleg = $this->make_space('delegated');
        wp_set_current_user($admin);
        $this->assertTrue(Mullion_REST_Base::current_actor_can_access_space($deleg), 'a System Admin accesses every space');
    }
    ...
    public function test_page_spaces_scoped_to_editor() {
        ...
        wp_set_current_user($editor);
        ob_start();
        Mullion_Embed::emit_page_spaces_js();
