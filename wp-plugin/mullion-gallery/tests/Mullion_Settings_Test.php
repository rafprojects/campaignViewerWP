<?php
/**
 * Tests for Mullion_Settings class.
 *
 * @package Mullion
 */

class Mullion_Settings_Test extends WP_UnitTestCase {
    ...
    public function test_get_setting_returns_individual_value() {
        ...
        $this->assertEquals('light', Mullion_Settings::get_setting('theme'));
        $this->assertEquals(24, Mullion_Settings::get_setting('items_per_page'));
        // Unset value should return default.
        $this->assertEquals('wp-jwt', Mullion_Settings::get_setting('auth_provider'));
    }
    ...
    public function test_get_setting_respects_custom_default() {
        ...
        $value = Mullion_Settings::get_setting('nonexistent_key', 'custom_default');
    ...
    public function test_sanitize_settings_validates_auth_provider() {
        ...
        $sanitized = Mullion_Settings::sanitize_settings($input);
    ...
    public function test_sanitize_gallery_overrides_persists_spotlight_adapter_settings() {
        $overrides = Mullion_Settings_Sanitizer::sanitize_gallery_overrides([
            'mode' => 'unified',
            'breakpoints' => [
                'desktop' => [
                    'unified' => [
    ...
    public function test_sanitize_gallery_config_payload_applies_field_level_rules() {
        ...
        $sanitized = Mullion_Settings_Sanitizer::sanitize_gallery_config_payload([
            'mode' => 'per-type',
            'breakpoints' => [
                'desktop' => [
                    'image' => [
    ...
    public function test_sanitize_gallery_overrides_rejects_invalid_nested_scalar_and_color_values() {
        $sanitized = Mullion_Settings_Sanitizer::sanitize_gallery_overrides([
            'mode' => 'per-type',
            'breakpoints' => [
                'desktop' => [
                    'image' => [
    ...
    public function test_sanitize_gallery_overrides_handles_campaign_payloads() {
        $sanitized = Mullion_Settings_Sanitizer::sanitize_gallery_overrides(wp_json_encode([
            'mode' => 'per-type',
            'breakpoints' => [
                'desktop' => [
                    'image' => [
    ...
    public function test_filter_auth_provider_returns_setting() {
        update_option(Mullion_Settings::OPTION_NAME, ['auth_provider' => 'none']);

        $result = Mullion_Settings_Service::filter_auth_provider('wp-jwt');
    ...
    public function test_filter_api_base_returns_setting_or_default() {
        ...
        delete_option(Mullion_Settings::OPTION_NAME);
        $result = Mullion_Settings_Service::filter_api_base('https://default.com');
        $this->assertEquals('https://default.com', $result);

        // Non-empty api_base should override.
        update_option(Mullion_Settings::OPTION_NAME, ['api_base' => 'https://custom.com']);
        $result = Mullion_Settings_Service::filter_api_base('https://default.com');
