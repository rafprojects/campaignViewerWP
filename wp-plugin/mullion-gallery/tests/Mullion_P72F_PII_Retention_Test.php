 ...
 */
class Mullion_P72F_PII_Retention_Test extends WP_UnitTestCase {
    ...
    private function reset(): void {
        ...
        wp_clear_scheduled_hook(Mullion_Maintenance::ACCESS_REQUESTS_PURGE_HOOK);
        wp_clear_scheduled_hook(Mullion_Maintenance::AUDIT_LOG_PURGE_HOOK);
    ...
    private function set_retention(string $key, int $days): void {
        update_option(Mullion_Settings::OPTION_NAME, [$key => $days]);
    }
    ...
    private function count_access_requests(): int {
        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL
        return (int) $wpdb->get_var('SELECT COUNT(*) FROM ' . Mullion_DB::get_access_requests_table());
    }

    private function count_audit_entries(): int {
        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL
        return (int) $wpdb->get_var('SELECT COUNT(*) FROM ' . Mullion_DB::get_audit_log_table());
    }
    ...
    public function test_purge_access_requests_removes_only_rows_past_window() {
        ...
        Mullion_Maintenance::purge_old_access_requests();

        $this->assertSame(1, $this->count_access_requests(), 'only the row older than 30 days should be purged');

        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL
        $remaining = $wpdb->get_var('SELECT email FROM ' . Mullion_DB::get_access_requests_table());
    ...
    public function test_purge_audit_log_removes_only_rows_past_window() {
        ...
        Mullion_Maintenance::purge_old_audit_log();

        $this->assertSame(1, $this->count_audit_entries(), 'only the row older than 30 days should be purged');

        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL
        $remaining = $wpdb->get_var('SELECT actor_login FROM ' . Mullion_DB::get_audit_log_table());
    ...
    public function test_register_schedules_purge_hooks_when_windows_set() {
        ...
        $this->assertNotFalse(wp_next_scheduled(Mullion_Maintenance::ACCESS_REQUESTS_PURGE_HOOK));
        $this->assertNotFalse(wp_next_scheduled(Mullion_Maintenance::AUDIT_LOG_PURGE_HOOK));
    }
    ...
    public function test_register_clears_purge_hooks_when_windows_zero() {
        ...
        wp_schedule_event(time() + 3600, 'weekly', Mullion_Maintenance::ACCESS_REQUESTS_PURGE_HOOK);
        wp_schedule_event(time() + 3600, 'weekly', Mullion_Maintenance::AUDIT_LOG_PURGE_HOOK);
        update_option(Mullion_Settings::OPTION_NAME, [
        ...
        $this->assertFalse(wp_next_scheduled(Mullion_Maintenance::ACCESS_REQUESTS_PURGE_HOOK));
        $this->assertFalse(wp_next_scheduled(Mullion_Maintenance::AUDIT_LOG_PURGE_HOOK));
    }
}
