<?php
/**
 * Tests for sanitize_card_config_payload() in Mullion_Settings_Sanitizer.
 *
 * @package Mullion
 */

class Mullion_Card_Config_Sanitizer_Test extends WP_UnitTestCase {

    // ── Invalid / empty inputs ────────────────────────────────────────────

    public function test_returns_default_for_non_array_non_string() {
        $result = Mullion_Settings_Sanitizer::sanitize_card_config_payload(42);
        // Default param is [] so fallback returns [].
        $this->assertEquals([], $result);
    }

    public function test_returns_default_for_invalid_json_string() {
        $result = Mullion_Settings_Sanitizer::sanitize_card_config_payload('{bad json');
        $this->assertEquals([], $result);
    }

    public function test_returns_empty_breakpoints_for_missing_breakpoints_key() {
        $result = Mullion_Settings_Sanitizer::sanitize_card_config_payload(['foo' => 'bar']);
        $this->assertEquals(['breakpoints' => []], $result);
    }

    public function test_returns_empty_breakpoints_for_null_input() {
        $result = Mullion_Settings_Sanitizer::sanitize_card_config_payload(null);
        $this->assertEquals([], $result);
    }

    public function test_returns_custom_default_when_provided() {
        $custom = ['breakpoints' => ['desktop' => ['cardScale' => 1.5]]];
        $result = Mullion_Settings_Sanitizer::sanitize_card_config_payload(false, $custom);
    ...
    public function test_accepts_valid_integer_override() {
        ...
        $result = Mullion_Settings_Sanitizer::sanitize_card_config_payload($input);
    ...
    public function test_accepts_json_string_input() {
        ...
        $result = Mullion_Settings_Sanitizer::sanitize_card_config_payload($json);
