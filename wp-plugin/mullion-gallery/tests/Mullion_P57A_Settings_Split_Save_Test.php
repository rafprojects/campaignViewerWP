...
class Mullion_P57A_Settings_Split_Save_Test extends WP_UnitTestCase {
    ...
    private function make_space(array $overrides = []): int {
        return Mullion_DB::insert_space([
    ...
    private function persisted_overrides(int $space_id): array {
        ...
        $json = $wpdb->get_var($wpdb->prepare(
            'SELECT settings_overrides FROM ' . Mullion_DB::get_spaces_table() . ' WHERE id = %d',
            $space_id
        ));
    ...
    public function test_settings_panel_animation_is_a_global_validated_enum() {
        ...
        $valid_options = Mullion_Settings_Registry::get_valid_options();
        $overridable   = Mullion_Settings::get_overridable_keys();
    ...
    public function test_global_only_key_from_space_panel_routes_to_global_option() {
        ...
        $this->assertSame(
            'fade',
            Mullion_Settings::get_settings()['settings_panel_animation'],
    ...
    public function test_split_routes_overridable_to_space_and_global_to_option() {
        ...
        $this->assertSame('scale', Mullion_Settings::get_settings()['settings_panel_animation'], 'global key → global option');
    }
    ...
    public function test_editor_may_write_a_non_admin_global_key_via_space_panel() {
        ...
        $this->assertNotContains('settings_panel_animation', Mullion_Settings_Registry::get_admin_only_fields());
    ...
    public function test_editor_gets_403_on_admin_only_global_via_space_panel() {
        ...
        $this->assertContains('cache_ttl', Mullion_Settings_Registry::get_admin_only_fields());
        $cache_ttl_before = Mullion_Settings::get_settings()['cache_ttl'];
        ...
        $this->assertSame(
            $cache_ttl_before,
            Mullion_Settings::get_settings()['cache_ttl'],
    ...
    private function full_panel_payload(array $changes = []): array {
        $payload = Mullion_Settings::to_js(Mullion_Settings::get_settings(), true);
    ...
    public function test_editor_full_payload_roundtrip_on_global_settings_endpoint() {
        ...
        $this->assertSame('github-light', Mullion_Settings::get_settings()['theme']);
    }
}
