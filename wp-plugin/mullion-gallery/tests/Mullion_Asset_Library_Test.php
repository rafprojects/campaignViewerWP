<?php
/**
 * Tests for Mullion_Asset_Library class (P41-OL1: DB-backed storage).
 *
 * @package Mullion
 */

class Mullion_Asset_Library_Test extends WP_UnitTestCase {

    public function setUp(): void {
        parent::setUp();
        // Ensure the overlays table exists for each test.
        Mullion_DB::maybe_create_assets_table();
    }
    ...
    public function test_get_all_returns_empty_array_when_no_entries() {
        $all = Mullion_Asset_Library::get_all();
    ...
    public function test_get_all_maps_uploaded_at_to_uploadedAt_key() {
        Mullion_Asset_Library::add( [ 'url' => 'https://example.com/o.png', 'name' => 'Test' ] );
        $all = Mullion_Asset_Library::get_all();
    ...
    public function test_add_returns_entry_with_id_url_name_uploadedAt() {
        $entry = Mullion_Asset_Library::add( [
    ...
    public function test_add_defaults_name_to_empty_string_when_missing() {
        $entry = Mullion_Asset_Library::add( [ 'url' => 'https://example.com/o.png' ] );
        $this->assertEquals( '', $entry['name'] );
    }

    public function test_add_persists_to_db() {
        Mullion_Asset_Library::add( [ 'url' => 'https://example.com/p.png', 'name' => 'Persist' ] );
        $all = Mullion_Asset_Library::get_all();
        $this->assertCount( 1, $all );
    }

    public function test_add_multiple_entries_are_all_returned() {
        Mullion_Asset_Library::add( [ 'url' => 'https://example.com/1.png', 'name' => 'One' ] );
        Mullion_Asset_Library::add( [ 'url' => 'https://example.com/2.png', 'name' => 'Two' ] );
        Mullion_Asset_Library::add( [ 'url' => 'https://example.com/3.png', 'name' => 'Three' ] );

        $this->assertCount( 3, Mullion_Asset_Library::get_all() );
    }

    // ── remove ──────────────────────────────────────────────────

    public function test_remove_returns_true_and_deletes_entry() {
        $entry = Mullion_Asset_Library::add( [ 'url' => 'https://example.com/r.png', 'name' => 'Removable' ] );

        $result = Mullion_Asset_Library::remove( $entry['id'] );

        $this->assertTrue( $result );
        $this->assertCount( 0, Mullion_Asset_Library::get_all() );
    }

    public function test_remove_returns_false_for_nonexistent_id() {
        $result = Mullion_Asset_Library::remove( 'nonexistent-id' );
        $this->assertFalse( $result );
    }

    public function test_remove_only_deletes_targeted_entry() {
        $e1 = Mullion_Asset_Library::add( [ 'url' => 'https://example.com/a.png', 'name' => 'Keep' ] );
        $e2 = Mullion_Asset_Library::add( [ 'url' => 'https://example.com/b.png', 'name' => 'Remove' ] );

        Mullion_Asset_Library::remove( $e2['id'] );

        $remaining = Mullion_Asset_Library::get_all();
        $this->assertCount( 1, $remaining );
        $this->assertEquals( $e1['id'], $remaining[0]['id'] );
    }

    public function test_remove_returns_false_when_table_is_empty() {
        $result = Mullion_Asset_Library::remove( 'any-id' );
        $this->assertFalse( $result );
    }
