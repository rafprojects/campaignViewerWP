 ...
 */
class Mullion_P66C_Scoped_Space_Id_Test extends WP_UnitTestCase {
    ...
    public function tearDown(): void {
        global $wpdb;
        $wpdb->query('DELETE FROM ' . Mullion_DB::get_analytics_table());
    ...
    public function test_analytics_event_is_stamped_with_space_id() {
        ...
        $stored = (int) $wpdb->get_var($wpdb->prepare(
            'SELECT space_id FROM ' . Mullion_DB::get_analytics_table() . ' WHERE campaign_id = %d LIMIT 1',
            $campaign_id
        ));
    ...
    public function test_sync_media_refs_stamps_space_id() {
        ...
        Mullion_DB::sync_media_refs($campaign_id, [['id' => 'm1'], ['id' => 'm2']]);

        global $wpdb;
        $rows = $wpdb->get_col($wpdb->prepare(
            'SELECT space_id FROM ' . Mullion_DB::get_media_refs_table() . ' WHERE campaign_id = %d',
            $campaign_id
        ));
    ...
    public function test_insert_access_request_stamps_space_id() {
        ...
        global $wpdb;
        $stored = (int) $wpdb->get_var($wpdb->prepare(
            'SELECT space_id FROM ' . Mullion_DB::get_access_requests_table() . ' WHERE token = %s',
            'tok-p66c'
        ));
        $this->assertSame($space_id, $stored);
    }
    ...
    public function test_backfill_stamps_historical_zero_space_rows() {
        ...
        $wpdb->insert(Mullion_DB::get_analytics_table(), [
            'campaign_id'  => $campaign_id,
            'event_type'   => 'view',
