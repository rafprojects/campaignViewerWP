 ...
 */
class Mullion_P66D_Duplicate_Space_Taxonomy_Test extends WP_UnitTestCase {
    ...
    public function test_duplicate_keeps_space_id() {
        ...
        $new_id = Mullion_Campaign_Duplicator::duplicate($source, 'Copy', false, false);
    ...
    public function test_duplicate_copies_category_and_tag_terms() {
        ...
        $cat = wp_insert_term('P66D Category', 'mullion_campaign_category');
        $tag = wp_insert_term('P66D Tag', 'mullion_campaign_tag');
        wp_set_object_terms($source, [(int) $cat['term_id']], 'mullion_campaign_category');
        wp_set_object_terms($source, [(int) $tag['term_id']], 'mullion_campaign_tag');

        $new_id = Mullion_Campaign_Duplicator::duplicate($source, 'Copy', false, false);

        $new_cats = wp_get_post_terms($new_id, 'mullion_campaign_category', ['fields' => 'ids']);
        $new_tags = wp_get_post_terms($new_id, 'mullion_campaign_tag', ['fields' => 'ids']);
    ...
    public function test_duplicate_without_terms_is_harmless() {
        ...
        $this->assertIsInt($new_id);
        $this->assertSame([], wp_get_post_terms($new_id, 'mullion_campaign_category', ['fields' => 'ids']));
    }
}
