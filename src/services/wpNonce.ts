 ...
 * (`window.__MULLION_CONFIG__.restNonce` / `window.__MULLION_REST_NONCE__`). The HTTP
 * transport takes these as injected callbacks (`getNonce`/`setNonce`/`noncePath`
 * on `ApiClientOptions`) so it stays free of any direct `window.__MULLION_*`
...
export function getWpNonce(): string | undefined {
  ...
  return window.__MULLION_CONFIG__?.restNonce ?? window.__MULLION_REST_NONCE__;
}

/** Persist a refreshed nonce back to both WP global slots. */
export function setWpNonce(nonce: string): void {
  if (typeof window === 'undefined') return;
  if (window.__MULLION_CONFIG__) {
    window.__MULLION_CONFIG__.restNonce = nonce;
  }
  (window as Window & { __MULLION_REST_NONCE__?: string }).__MULLION_REST_NONCE__ = nonce;
}
