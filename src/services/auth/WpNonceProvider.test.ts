...
describe('WpNonceProvider', () => {
  ...
  it('init() detects an admin user and caches user + permissions', async () => {
    (globalThis.fetch as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true,
      json: async () => ({ campaignIds: ['7', '9'], isAdmin: true, userId: 3, userEmail: 'a@b.c' }),
    });
    ...
    expect(await provider.getUser()).toEqual({ id: '3', email: 'a@b.c', role: 'admin' });
