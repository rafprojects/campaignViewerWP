...
type Win = typeof window & { __MULLION_PAGE_SPACES__?: PageSpace[] };
...
const SPACES: PageSpace[] = [
  { instanceId: 'mullion-hero-0', id: 1, slug: 'hero', name: 'Hero Gallery' },
  { instanceId: 'mullion-products-0', id: 2, slug: 'products', name: 'Products' },
];

describe('usePageSpaces', () => {
  it('returns an empty array when window.__MULLION_PAGE_SPACES__ is not defined', () => {
  ...
  it('returns the correct length when one space is registered', () => {
    (window as Win).__MULLION_PAGE_SPACES__ = [SPACES[0]!];
    const { result } = renderHook(() => usePageSpaces());
