...
describe('VideoCarousel', () => {
  ...
  it('uses configured manual video height string in manual mode', () => {
    render(
      <VideoCarousel
        videos={videos}
        settings={{
  ...
  it('uses aspect ratio sizing in auto mode', () => {
    ...
    expect(screen.getByTestId('video-player-frame')).toHaveStyle({
      height: 'auto',
      aspectRatio: '16 / 9',
    });
  });

  it('restrains video height to the viewport when viewport mode is selected', () => {
    render(
      <VideoCarousel
        videos={videos}
        settings={{
    ...
    expect(screen.getByTestId('video-player-frame')).toHaveStyle({
      ...
      maxWidth: 'min(calc(min(calc(100dvh - 16rem), 68dvh) * 1.7777777777777777), 72vw)',
      aspectRatio: '16 / 9',
    });
  });

  it('combines modal width caps with viewport restraint when both are present', () => {
    render(
      <VideoCarousel
        videos={videos}
        maxWidth={900}
    ...
    expect(screen.getByTestId('video-player-frame')).toHaveStyle({
      maxWidth: 'min(900px, calc(min(calc(100dvh - 16rem), 68dvh) * 1.7777777777777777), 72vw)',
    });
  });
});
