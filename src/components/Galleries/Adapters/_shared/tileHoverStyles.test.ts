...
describe('tileHoverStyles — rotation-composed bounce (B-7)', () => {
  it('buildTileStyles bounce keyframes compose the slot rotation custom property', () => {
    ...
    expect(css).toContain('@keyframes mullion-tile-lb-bounce');
    expect(css).toContain('rotate(var(--mullion-slot-rot, 0deg)) scale(1.07)');
    expect(css).toContain('rotate(var(--mullion-slot-rot, 0deg)) scale(1)');
  ...
  it('buildBoxShadowStyles bounce keyframes compose the slot rotation custom property', () => {
    ...
    expect(css).toContain('@keyframes mullion-tile-lb-rect-bounce');
    expect(css).toContain('rotate(var(--mullion-slot-rot, 0deg)) scale(1.06)');
    expect(css).not.toContain('transform: scale(');
  });
});
